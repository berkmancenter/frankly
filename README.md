...
- `data_models/lib/firestore`
    These are the models defined in Firestore. Firestore is the database behind this app.
...
2. To install all Dart dependencies run `flutter pub get`.
...
- app.domain: The domain for your app without the protocol (i.e. "www.example.com")
...
The emulators are configured with a file, rather than running `functions:config:set.` To configure the emulators, create the file `firebase/functions/.runtimeconfig.json` A sample file containing the config properties described above can be found in `firebase/functions/.runtimeconfig.json.local.example`
...
See instructions [here](e2e/README.md) for developing and running end-to-end Playwright tests.
...
# Hosting Your Own Instance of Frankly
