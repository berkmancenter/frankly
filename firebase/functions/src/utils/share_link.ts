    ...
    private async _getHtmlContent(requestedUri: string, isLinkedIn = false): Promise<string> {
        ...
        if (eventMatch) {
            ...
            const templateDoc = await firestore
                .doc(`community/${communityId}/templates/${templateId}`)
                .get()
            const template = TemplateUtils.templateFromSnapshot(templateDoc)

            const eventDoc = await firestore
                .doc(`community/${communityId}/templates/${templateId}/events/${eventId}`)
                .get()
            const event = {
    ...
    async expressAction(
        ...
        try {
            ...
            if (
                ...
                res.send(
                    await this._getHtmlContent(requestedUri, userAgent.includes('linkedinbot'))
                )
            } else {
