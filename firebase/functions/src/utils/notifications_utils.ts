...
import {
  ...
  NotificationEmailType, Event, Template, Participant, ParticipantStatus,
  SendGridEmail, SendGridEmailMessage, EventEmailType,
} from '../types';
...
export class NotificationsUtils {
  ...
  async sendEmailToEventParticipants({
    ...
    console.log('Sending event message email to:', emails.map((e) => e.to));
    await sendEmailClient.sendEmails(emails);
  }
