...
import { Community, Membership, MembershipStatus, PlanCapabilityList, PlanType, BillingSubscription, PartnerAgreement } from '../types';
...
export class SubscriptionPlanUtil {
  ...
  private async _getSubscriptionPlan({
    ...
    if (!activeSubscriptions.empty) {
      const subscriptions = activeSubscriptions.docs.map((doc) => doc.data() as BillingSubscription);
      const types = [...new Set(subscriptions.map((s) => s.type).filter(Boolean))] as string[];
      const plans = await Promise.all(types.map((t) => this._getCapabilitiesForType({ type: t })));
  ...
  private _combineCapabilities(a: PlanCapabilityList, b: PlanCapabilityList): PlanCapabilityList {
    ...
    return {
      ...
      hasSmartMatching: applyReducer((x, y) => x || y, a.hasSmartMatching, b.hasSmartMatching),
      ...
      hasAdvancedBranding: applyReducer((x, y) => x || y, a.hasAdvancedBranding, b.hasAdvancedBranding),
      hasBasicAnalytics: applyReducer((x, y) => x || y, a.hasBasicAnalytics, b.hasBasicAnalytics),
      hasCustomAnalytics: applyReducer((x, y) => x || y, a.hasCustomAnalytics, b.hasCustomAnalytics),
