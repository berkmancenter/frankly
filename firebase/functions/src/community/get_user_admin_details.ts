    ...
    async action(
        ...
        if (!authorized && request.communityId) {
            ...
            if (!authorized && request.eventPath) {
                ...
                authorized =
                    (membership?.status ? membershipIsAdmin(membership.status) : false) && allActive
            }
            ...
            if (!authorized) {
                const memberDocuments = await Promise.all(
                    request.userIds.map((id) =>
                        firestore
                            .doc(`memberships/${id}/community-membership/${request.communityId}`)
                            .get()
                    )
                )
                ...
                authorized =
                    (membership?.status ? membershipIsAdmin(membership.status) : false) &&
                    allAreMembersOrRequesters
            }
        }
        ...
        return {
            userAdminDetails: userRecords.map(
