...
import { UpdateMembershipRequest, Membership, MembershipStatus, Community } from '../types'
import {
    orElseUnauthorized,
    orElseInvalidArgument,
    orElseNotFound,
    jsonSubset,
} from '../utils/utils'
...
export class UpdateMembership extends OnCallMethod<UpdateMembershipRequest> {
    ...
    async action(
        ...
        const targetMembershipDoc = firestore.doc(
        ...
        const elevatingToAdmin =
            !adminStatuses.includes(currentStatus) && adminStatuses.includes(targetStatus!)
        const elevatingToFacilitator =
            !facilitatorStatuses.includes(currentStatus) &&
            facilitatorStatuses.includes(targetStatus!)
            ...
            if (elevatingToAdmin) {
                const [adminDocs, ownerDocs, modDocs] = await Promise.all([
                    ...
                    firestore
                        .collectionGroup('community-membership')
                        .where('communityId', '==', communityId)
                        .where('status', '==', MembershipStatus.owner)
                        .get(),
                    firestore
                        .collectionGroup('community-membership')
                        .where('communityId', '==', communityId)
                        .where('status', '==', MembershipStatus.moderator)
                        .get(),
                ])
        ...
        if (targetMembershipSnap.exists) {
            ...
            await targetMembershipDoc.set(
                firestoreUtils.toFirestoreJson(
                    updatedMembership as unknown as Record<string, unknown>
                ),
                { merge: true }
            )
        }
    ...
    private _isValidMemberUpdate(opts: {
        ...
        const { requesterStatus, currentStatus, targetStatus, requestingUserId, targetUserId } =
            opts
