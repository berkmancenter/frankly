...
export class OnCommunity extends OnFirestoreFunction<Community> {
  constructor() {
    super(
      [{ functionName: 'CommunityOnCreate', firestoreEventType: FirestoreEventType.onCreate }],
      (snapshot) => {
        const data = firestoreUtils.fromFirestoreJson(snapshot.data() ?? {}) as unknown as Community;
