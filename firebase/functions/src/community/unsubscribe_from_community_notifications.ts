...
import {
  UnsubscribeFromCommunityNotificationsRequest, Membership, MembershipStatus,
  CommunityUserSettings, NotificationEmailType,
} from '../types';
...
export class UnsubscribeFromCommunityNotifications extends OnCallMethod<UnsubscribeFromCommunityNotificationsRequest> {
  ...
  async action(
    ...
    const communityIds = membershipsData.docs.map((doc) => (doc.data() as Membership).communityId);
    ...
    await Promise.all(
      communityIds.map((communityId) => {
        ...
        return firestore
          ...
          .set(firestoreUtils.toFirestoreJson(settings as unknown as Record<string, unknown>), { merge: true });
      })
    );
  }
}
