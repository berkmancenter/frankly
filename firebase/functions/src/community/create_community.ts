...
import {
  ...
  Community, Membership, MembershipStatus, PartnerAgreement,
  CommunitySettings,
} from '../types';
...
export class CreateCommunity extends OnCallMethod<CreateCommunityRequest> {
  ...
  async action(
    ...
    community = {
      ...
      creatorId: userId,
      profileImageUrl: community.profileImageUrl ?? `https://picsum.photos/seed/${communityDocRef.id}-profile/512`,
    ...
    await firestore.runTransaction(async (transaction) => {
      ...
      if (doc.exists) {
        throw new HttpsError('failed-precondition', `Community with ID ${communityDocRef.id} already exists.`);
      }

      transaction.set(communityDocRef, firestoreUtils.toFirestoreJson(community as unknown as Record<string, unknown>));
      ...
      transaction.set(
        agreementRef,
        jsonSubset(agreementChangedFields, firestoreUtils.toFirestoreJson(agreementUpdated as unknown as Record<string, unknown>)),
        { merge: true }
      );
    });
