...
import { CreateAnnouncementRequest, Membership, CommunityUserSettings, NotificationEmailType, SendGridEmailMessage } from '../types';
...
export class CreateAnnouncement extends OnCallMethod<CreateAnnouncementRequest> {
  ...
  async action(
    ...
    await docRef.set(firestoreUtils.toFirestoreJson(request.announcement as unknown as Record<string, unknown>));
