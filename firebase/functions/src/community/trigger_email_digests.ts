...
import {
  Community, Membership, MembershipStatus, CommunityUserSettings,
  NotificationEmailType, Event, EventStatus, SendGridEmail, SendGridEmailMessage,
} from '../types';
...
export class TriggerEmailDigests {
  ...
  async action(_context: functions.EventContext): Promise<void> {
    ...
    console.log('Generating emails for communities:', eventsListFiltered.map((d) => `${d.community.id}: ${d.events.length} events`));
      ...
      communities.map(async (community) => {
        ...
        const events = await Promise.all(
          eventDocs.docs.map(async (doc) => {
            const event = { ...(firestoreUtils.fromFirestoreJson(doc.data()) as unknown as Event), id: doc.id };
          ...
          members.map(async (membership) => {
            try {
              ...
              const settingsDoc = await firestore
                .doc(`privateUserData/${membership.userId}/communityUserSettings/${community.id}`)
                .get();
              ...
              emails.push({
                ...
                message: {
                  ...
                  html: `<p>You have ${events.length} upcoming events in ${community.name}!</p>
                         <p><a href="${getUnsubscribeUrl({ userId: user.uid })}">Unsubscribe</a></p>`,
                },
              });
            } catch (e) {
              console.error(`Error processing email for user ${membership.userId}:`, e);
            }
          })
        );
      })
