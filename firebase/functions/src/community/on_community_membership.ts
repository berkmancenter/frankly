...
export class OnCommunityMembership extends OnFirestoreFunction<Membership> {
  constructor() {
    super(
      [{ functionName: 'CommunityMembershipOnCreate', firestoreEventType: FirestoreEventType.onCreate }],
      (snapshot) => {
        return firestoreUtils.fromFirestoreJson(snapshot.data() ?? {}) as unknown as Membership;
      }
    );
  }
  ...
  private async _updateOnboardingSteps(
    ...
    if (creatorId === membership.userId) {
      console.log(`User (${membership.userId}) is a creator of the Community (${communityId})`);
      return;
    }
    ...
    if (!isUserInCommunity) {
      console.log(`User (${membership.userId}) is not in community (${communityId}). Something is wrong`);
      return;
    }
