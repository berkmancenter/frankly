...
const baseAllowedUpdateFields = new Set([
  'name', 'contactEmail', 'profileImageUrl', 'bannerImageUrl', 'isPublic',
  'description', 'tagLine', 'websiteUrl', 'facebookUrl', 'linkedinUrl',
  'twitterUrl', 'blueskyUrl', 'youtubeUrl', 'instagramUrl',
  'communitySettings', 'eventSettings', 'donationDialogText', 'ratingSurveyUrl',
  'themeLightColor', 'themeDarkColor', 'onboardingSteps',
]);
  ...
  async action(
    ...
    for (const displayId of community.displayIds ?? []) {
      ...
      if (!existing.empty && existing.docs.some((doc) => doc.id !== community.id)) {
        throw new HttpsError('failed-precondition', `The URL display name ${displayId} is already taken.`);
      }
    }
    ...
    try {
      const finalisedUpdateMap = jsonSubset(updateFields, firestoreUtils.toFirestoreJson(community as unknown as Record<string, unknown>));
