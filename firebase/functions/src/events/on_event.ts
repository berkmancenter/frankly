...
export class OnEvent extends OnFirestoreFunction<Event> {
  ...
  async onUpdate(
    ...
    await Promise.all([
      this._swallowErrors(() => this._checkHostlessUpdates(before, after, updateTime, context), 'check hostless update'),
      this._swallowErrors(() => this._sendEmailUpdates(before, after, updateTime, context), 'send email updates'),
    ]);
  }
  ...
  private async _sendEmailUpdates(
    ...
    if (!(after.eventSettings?.reminderEmails ?? community.eventSettingsMigration?.reminderEmails ?? true)) return;
  ...
  private async _checkHostlessUpdates(
    ...
    if (getEventType(after) === EventType.hostless && (eventTypeChanged || waitingRoomFinishedTimeChanged)) {
