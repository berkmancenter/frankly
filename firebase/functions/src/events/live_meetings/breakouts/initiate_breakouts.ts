...
export class InitiateBreakouts extends OnCallMethod<InitiateBreakoutsRequest> {
  constructor() {
    super(
      'InitiateBreakouts',
      (json) => json as InitiateBreakoutsRequest,
      { runWithOptions: { timeoutSeconds: 120, memory: '4GB', minInstances: 0 } }
    );
  }

  async action(request: InitiateBreakoutsRequest, context: functions.https.CallableContext): Promise<void> {
  ...
  private async _pingBreakoutsAvailability({
    ...
    const newlyInitiated = await firestore.runTransaction(async (transaction) => {
      ...
      if ((liveMeeting as any).currentBreakoutSession?.breakoutRoomSessionId === breakoutRoomSessionId) {
      ...
      transaction.set(liveMeetingRef, { currentBreakoutSession: firestoreUtils.toFirestoreJson(breakoutSession as Record<string, unknown>) }, { merge: true });
      return true;
    });

    if (newlyInitiated) {
      console.log('scheduling assign to breakouts server check');
      const { CheckAssignToBreakoutsServer } = await import('./check_assign_to_breakouts_server');
