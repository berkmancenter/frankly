...
export class CheckHostlessGoToBreakouts extends OnCallMethod<CheckHostlessGoToBreakoutsRequest> {
    ...
    async action(
        request: CheckHostlessGoToBreakoutsRequest,
        context: functions.https.CallableContext
    ): Promise<void> {
        await this.checkHostlessGoToBreakouts(request, context.auth!.uid)
    }

    async checkHostlessGoToBreakouts(
        request: CheckHostlessGoToBreakoutsRequest,
        userId: string
    ): Promise<void> {
        ...
        if (
            (liveMeeting as any).currentBreakoutSession?.breakoutRoomStatus ===
        ...
        await new InitiateBreakouts().initiateBreakouts({
            ...
            request: {
                ...
                assignmentMethod:
                    (event as any).breakoutRoomDefinition?.assignmentMethod ??
                    BreakoutAssignmentMethod.targetPerRoom,
                targetParticipantsPerRoom:
                    (event as any).breakoutRoomDefinition?.targetParticipants ??
