...
export abstract class OnRequestMethod<T> {
    ...
    async schedule(request: T, scheduledTime: Date): Promise<void> {
        ...
        // on_request_method → scheduled_functions → extend_cloud_task_scheduler → on_request_method
        const { scheduledFunctions } = await import('./utils/infra/scheduled_functions')
