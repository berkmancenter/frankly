...
/**
 * Test that a user who has not been manually given permission will not see the start community button
 * (Note this test assumes that the user defined as MEMBER does NOT has this permission)
 */
test('test user cannot start a public community', async ({ page }) => {
    await signOut(page);
    await signInAsMember(page);
    new HomePage(page).assertStartCommunityNotVisible();
});
