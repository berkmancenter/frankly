...
test('test moderator can add an image agenda item to event agenda', async ({ page }) => {
    ...
    await agendaTab.clickAddCard();
    await agendaTab.selectCardType('Image');
    await agendaTab.enterCardTitle('Does this look good to you?');
    await agendaTab.selectAndCropCardImage(
        'https://upload.wikimedia.org/wikipedia/commons/e/ea/Pizza_with_pineapple.jpg'
    );
    await agendaTab.clickSaveCard();
});
