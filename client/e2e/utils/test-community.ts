...
export const TEMPLATES_URL = '/discuss';
export const EVENTS_URL = '/discuss/upcoming';
...
export const gotoTestCommunitySpace = async (page: Page) => {
    ...
    commSpace.setUrl(process.env.TEST_COMMUNITY_URL!);
...
export const gotoTestCommunityTemplatesPage = async (page: Page) => {
    const templatesPage = new TemplatesPage(
        page,
        process.env.TEST_COMMUNITY_URL!.concat(TEMPLATES_URL)
    );
...
export const gotoTestCommunityEventsPage = async (page: Page) => {
    const eventsPage = new EventsPage(page, process.env.TEST_COMMUNITY_URL!.concat(EVENTS_URL));
...
export const gotoTestCommunityDiscussionBoard = async (page: Page) => {
    const postsPage = new PostsPage(page, process.env.TEST_COMMUNITY_URL!.concat(POSTS_URL));
...
export const createTemplate = async (
    ...
    templatePage.setUrl(page.url());
    return templatePage;
};

/**
 * Removes the specified template. Assumes you are currently on the template page
 */
export const removeTemplate = async (templatePage: TemplatePage) => {
