...
import { gotoTestCommunitySpace } from './test-community';
...
export const createEvent = async (
    ...
    const eventPage = new EventPage(page, templateName, eventDate, eventPublic);
    // give time for URL to update
    await expect(page).toHaveURL(/.*\/discuss\/.+\/.+/);
    eventPage.setUrl(page.url());
...
export const enterEventInNewContext = async (
    ...
    await commSpace.clickUpcomingConversation(templateName, shortEventDate);
