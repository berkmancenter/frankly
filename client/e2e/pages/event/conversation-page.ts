...
export class ConversationPage {
    ...
    private readonly startConversationButton: Locator;
    ...
    private readonly cardContentField: Locator;
    ...
    constructor(page: Page) {
        ...
        this.startConversationButton = page.getByRole('button', { name: 'Start event' });
        ...
        this.cardTitleField = page.getByLabel('Title', { exact: true });
        this.cardContentField = page.getByLabel('Content');
    ...
    async enterCardTitle(title: string) {
        await this.cardTitleField.focus();
        await this.cardTitleField.click();
        await this.cardTitleField.fill(title);
    }

    async enterCardContent(content: string) {
        await this.cardContentField.click();
        await this.cardContentField.fill(content);
    }

    async assertAgendaTopicVisible(itemheading: string) {
        await expect(this.page.getByLabel(new RegExp(itemheading.concat('.*')))).toBeVisible();
    }

    async assertTopicCardVisible(itemheading: string) {
        await expect(
