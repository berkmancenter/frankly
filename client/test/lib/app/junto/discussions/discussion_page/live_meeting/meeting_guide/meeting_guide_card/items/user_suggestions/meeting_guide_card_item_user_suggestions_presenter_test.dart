...
import 'package:junto/app/junto/discussions/discussion_page/live_meeting/meeting_guide/meeting_guide_card/items/user_suggestions/meeting_guide_card_item_user_suggestions_model.dart';
import 'package:junto/app/junto/discussions/discussion_page/live_meeting/meeting_guide/meeting_guide_card/items/user_suggestions/meeting_guide_card_item_user_suggestions_presenter.dart';
...
void main() {
  ...
  final mockDiscussionPermissions = MockDiscussionPermissionsProvider();
  ...
  setUp(() {
    ...
    presenter = MeetingGuideCardItemUserSuggestionsPresenter(
      ...
      discussionPermissions: mockDiscussionPermissions,
    );
  });
  ...
  test('getParticipantAgendaItemDetailsStream', () {
    ...
    when(mockMeetingGuideCardStore.participantAgendaItemDetailsStream).thenAnswer((_) => stream);
  ...
  group('addSuggestion', () {
    test('suggestion is empty', () async {
      ...
      verify(mockView.showMessage('Suggestion cannot be empty', toastType: ToastType.failed))
          .called(1);
    ...
    test('success', () async {
      ...
      when(mockMeetingGuideCardStore.meetingGuideCardAgendaItem).thenReturn(agendaItem);
  ...
  test('removeSuggestion', () async {
    final agendaItem = AgendaItem(id: 'agendaItemId');
    final meetingUserSuggestion = MeetingUserSuggestion(id: 'id', suggestion: 'suggestion');
    when(mockMeetingGuideCardStore.meetingGuideCardAgendaItem).thenReturn(agendaItem);
    when(mockUserService.currentUserId).thenReturn('userId');
    when(mockAgendaProvider.liveMeetingPath).thenReturn('path');
  ...
  group('getFormattedDetails', () {
    ...
    test('success', () {
      ...
      final user1Item2 = MeetingUserSuggestion(id: 'id2', suggestion: 'suggestion2');
      ...
      expect(
        result,
        [
          ParticipantAgendaItemDetails(userId: 'userId2', suggestions: [user1Item1]),
          ParticipantAgendaItemDetails(userId: 'userId3', suggestions: [user3Item1]),
          ParticipantAgendaItemDetails(userId: 'userId2', suggestions: [user1Item2]),
          ParticipantAgendaItemDetails(userId: 'userId2', suggestions: [user1Item3]),
        ],
      );
    });
  });
  ...
  group('isMySuggestion', () {
    final participantAgendaItemDetails = ParticipantAgendaItemDetails(userId: 'userId');
  ...
  group('toggleLikeDislike', () {
    group('like', () {
      test('is liked already', () async {
        final agendaItem = AgendaItem(id: 'agendaItemId');
        final participantItemDetails = ParticipantAgendaItemDetails(userId: 'userId');
        when(mockUserService.currentUserId).thenReturn('currentUserId');
        when(mockMeetingUserSuggestion.isLiked('currentUserId')).thenReturn(true);
        ...
        verify(
          mockFirestoreMeetingGuideService.toggleLikeInMeetingSuggestion(LikeType.neutral,
              agendaItemId: 'agendaItemId',
      ...
      test('is not liked already', () async {
        ...
        final participantItemDetails = ParticipantAgendaItemDetails(userId: 'userId');
        when(mockUserService.currentUserId).thenReturn('currentUserId');
        when(mockMeetingUserSuggestion.isLiked('currentUserId')).thenReturn(false);
        when(mockMeetingUserSuggestion.id).thenReturn('meetingUserSuggestionId');
        when(mockMeetingGuideCardStore.meetingGuideCardAgendaItem).thenReturn(agendaItem);
        when(mockAgendaProvider.liveMeetingPath).thenReturn('path');
        ...
        verify(
          mockFirestoreMeetingGuideService.toggleLikeInMeetingSuggestion(LikeType.like,
              agendaItemId: 'agendaItemId',
              voterId: 'currentUserId',
              creatorId: 'userId',
              liveMeetingPath: 'path',
    ...
    group('dislike', () {
      test('is disliked already', () async {
        ...
        final participantItemDetails = ParticipantAgendaItemDetails(userId: 'userId');
        when(mockUserService.currentUserId).thenReturn('currentUserId');
        when(mockMeetingUserSuggestion.isDisliked('currentUserId')).thenReturn(true);
        when(mockMeetingUserSuggestion.id).thenReturn('meetingUserSuggestionId');
        when(mockMeetingGuideCardStore.meetingGuideCardAgendaItem).thenReturn(agendaItem);
        when(mockAgendaProvider.liveMeetingPath).thenReturn('path');
      ...
      test('is not disliked already', () async {
        ...
        final participantItemDetails = ParticipantAgendaItemDetails(userId: 'userId');
        when(mockUserService.currentUserId).thenReturn('currentUserId');
        when(mockMeetingUserSuggestion.isDisliked('currentUserId')).thenReturn(false);
        when(mockMeetingUserSuggestion.id).thenReturn('meetingUserSuggestionId');
        when(mockMeetingGuideCardStore.meetingGuideCardAgendaItem).thenReturn(agendaItem);
        when(mockAgendaProvider.liveMeetingPath).thenReturn('path');
        ...
        verify(
          mockFirestoreMeetingGuideService.toggleLikeInMeetingSuggestion(LikeType.dislike,
              agendaItemId: 'agendaItemId',
              voterId: 'currentUserId',
              creatorId: 'userId',
              liveMeetingPath: 'path',
