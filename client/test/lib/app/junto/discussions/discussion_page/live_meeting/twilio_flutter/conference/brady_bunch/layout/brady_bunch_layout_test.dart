...
import 'package:junto/app/junto/discussions/discussion_page/live_meeting/video/conference/brady_bunch/layout/brady_bunch_layout.dart';

main() {
  double _getMaxDeadspace(int participantCount) {
    const resolution = 10;
    const minSize = Size(500, 200);
    const maxSize = Size(2000, 2000);
    double maxDeadspacePct = 0.0;

    for (int i = minSize.width ~/ resolution; i < maxSize.width ~/ resolution; i++) {
      for (int j = minSize.height ~/ resolution; j < maxSize.height ~/ resolution; j++) {
        final width = (resolution * i).toDouble();
        final height = (resolution * j).toDouble();
        final layout = BradyBunchLayout.calculateOptimalLayout(
            width: width, height: height, participantCount: participantCount);
  ...
  group(
    ...
    () {
      test('allParticipantsLaidOut', () {
        ...
        for (int p = 2; p <= 10; p++) {
          for (int i = minSize.width ~/ resolution; i < maxSize.width ~/ resolution; i++) {
            for (int j = minSize.height ~/ resolution; j < maxSize.height ~/ resolution; j++) {
              ...
              final height = (resolution * j).toDouble();
              final layout = BradyBunchLayout.calculateOptimalLayout(
                  width: width, height: height, participantCount: p);
        ...
        expect(allParticipantsLaidOut, isTrue,
            reason:
  ...
  group(
    ...
    () {
      test('imageSizeConsistentWithConstraints', () {
        ...
        for (int p = 2; p <= 10; p++) {
          for (int i = minSize.width ~/ resolution; i < maxSize.width ~/ resolution; i++) {
            for (int j = minSize.height ~/ resolution; j < maxSize.height ~/ resolution; j++) {
              ...
              final layoutTooWide = layout.layoutParameters.columns * imageSize.width >
                  (i * resolution).toDouble() + .01;
              final layoutTooTall =
                  layout.layoutParameters.rows * imageSize.height > j * resolution + .01;
              final layoutTooSmall =
                  (layout.layoutParameters.rows * imageSize.height) - (j * resolution) > .01 &&
              ...
              if (layoutTooWide || layoutTooTall || layoutTooSmall) {
                ...
                violatingLayout = layout;
                sizeToAccomodate = Size((i * resolution).toDouble(), (j * resolution).toDouble());
        ...
        expect(imageSizesConsistent, isTrue,
            reason:
