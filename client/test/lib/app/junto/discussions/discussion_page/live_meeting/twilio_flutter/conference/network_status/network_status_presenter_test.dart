...
import 'package:junto/app/junto/discussions/discussion_page/live_meeting/video/conference/networking_status/networking_status_model.dart';
import 'package:junto/app/junto/discussions/discussion_page/live_meeting/video/conference/networking_status/networking_status_presenter.dart';
...
void main() {
  ...
  setUp(() {
    ...
    when(mockRoom.discussionProvider).thenReturn(mockDiscussionProvider);
  ...
  group('updateNetworkQuality', () {
    group('_model.networkQualityLevel == QualityType.qualityBad && isVideoEnabled', () {
      test('network quality stays bad after 5s', () async {
        ...
        when(mockLocalParticipant.networkQualityLevel).thenReturn(QualityType.qualityBad);
    ...
    group(
      ...
      () {
        test('_model.isLowNetworkQuality', () {
          ...
          when(mockConferenceRoom.room).thenReturn(mockRoom);
          when(mockRoom.localParticipant).thenReturn(mockLocalParticipant);
          when(mockLocalParticipant.networkQualityLevel).thenReturn(QualityType.qualityGood);
          when(mockConferenceRoom.videoEnabled).thenReturn(true);
    ...
    group('_model.networkQualityLevel != QualityType.qualityBad && !isVideoEnabled', () {
      test('_model.isLowNetworkQuality', () {
        ...
        when(mockConferenceRoom.room).thenReturn(mockRoom);
        when(mockRoom.localParticipant).thenReturn(mockLocalParticipant);
        when(mockLocalParticipant.networkQualityLevel).thenReturn(QualityType.qualityPoor);
        when(mockConferenceRoom.videoEnabled).thenReturn(false);
  ...
  group('getCorrectWidget', () {
    ...
    test(
        ...
        () {
      model.isLowNetworkQualityMessageDismissed = true;
      final DateTime timeBefore = model.messageShowTimeThreshold.subtract(Duration(seconds: 1));
    ...
    test(
        ...
        () {
      model.isLowNetworkQualityMessageDismissed = true;
      final DateTime timeAfter = model.messageShowTimeThreshold.add(Duration(seconds: 1));
