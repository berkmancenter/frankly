...
void main() {
  ...
  final MockPrePostCardWidgetPresenterHelper mockHelper = MockPrePostCardWidgetPresenterHelper();
  final MockUserService mockUserService = MockUserService();
  final MockDiscussion mockDiscussion = MockDiscussion();
  late PrePostCardWidgetModel model =
      PrePostCardWidgetModel(PrePostCardType.preEvent, mockDiscussion, false, null);
  late PrePostCardWidgetPresenter presenter;

  setUp(() {
    model = PrePostCardWidgetModel(PrePostCardType.preEvent, mockDiscussion, false, null);
  ...
  group('validateButtonText', () {
    ...
    group('surveyUrl is not null', () {
      ...
      test('text is null', () {
        ...
        expect(presenter.validateButtonText(null, 0), 'Button text cannot be empty');
      });

      test('text is empty', () {
        model.prePostCard = prePostCard;

        expect(presenter.validateButtonText('', 0), 'Button text cannot be empty');
        expect(presenter.validateButtonText('      ', 0), 'Button text cannot be empty');
      });
  ...
  group('validateUrl', () {
    test('survey URL does not exist and additional attributes are added', () {
      final prePostCard = PrePostCard(
        ...
        prePostUrls: [
          PrePostUrlParams(
            ...
            attributes: [
              PrePostCardAttribute(type: PrePostCardAttributeType.email, queryParam: 'email')
            ],
          )
        ],
      );
      ...
      expect(presenter.validateUrl(null, 0), 'URL cannot be empty if some attributes are entered');
      expect(presenter.validateUrl('', 0), 'URL cannot be empty if some attributes are entered');
    });
  ...
  group('getAvailableAttributeTypes', () {
    test('0/3 selected, 3 retrieved', () {
      expect(presenter.getAvailableAttributeTypes([]), [
        ...
        PrePostCardAttributeType.discussionId,
    ...
    test('1/3 selected, 2 retrieved', () {
      final selectedAttributes = [
        PrePostCardAttribute(type: PrePostCardAttributeType.userId, queryParam: ''),
      ];
    ...
    test('2/3 selected, 1 retrieved', () {
      final selectedAttributes = [
        PrePostCardAttribute(type: PrePostCardAttributeType.userId, queryParam: ''),
        PrePostCardAttribute(type: PrePostCardAttributeType.discussionId, queryParam: ''),
      ];
    ...
    test('3/3 selected, 0 retrieved', () {
      final selectedAttributes = [
        PrePostCardAttribute(type: PrePostCardAttributeType.userId, queryParam: ''),
        PrePostCardAttribute(type: PrePostCardAttributeType.discussionId, queryParam: ''),
        PrePostCardAttribute(type: PrePostCardAttributeType.email, queryParam: '')
      ];
  ...
  group('addNewURLParamRow', () {
    test('something went wrong, no available attributes after clicking to add new row', () {
      final prePostCard = PrePostCard(
        type: PrePostCardType.preEvent,
        headline: 'headline',
        message: 'message',
        prePostUrls: [
          PrePostUrlParams(attributes: [
            PrePostCardAttribute(type: PrePostCardAttributeType.userId, queryParam: ''),
            PrePostCardAttribute(type: PrePostCardAttributeType.discussionId, queryParam: ''),
            PrePostCardAttribute(type: PrePostCardAttributeType.email, queryParam: ''),
          ])
  ...
  test('launchUrl', () async {
    ...
    model.discussion = mockDiscussion;
    model.email = 'email';
    when(mockPrePostCard.getFinalisedUrl(
            userId: 'userId',
  ...
  test('updateCard', () {
    final prePostCard = PrePostCard(
      ...
      prePostUrls: [
        PrePostUrlParams(
          attributes: [
            PrePostCardAttribute(type: PrePostCardAttributeType.userId, queryParam: 'userId'),
            PrePostCardAttribute(type: PrePostCardAttributeType.discussionId, queryParam: ''),
            PrePostCardAttribute(type: PrePostCardAttributeType.email, queryParam: 'email'),
          ],
        )
  ...
  group('updateAttributeTypeSelection', () {
    ...
    test('selectedType is not null', () {
          ...
          PrePostUrlParams(
            attributes: [
              ...
              PrePostCardAttribute(
                  type: PrePostCardAttributeType.discussionId, queryParam: 'email'),
            ],
          )
        ],
      );
      ...
      presenter.updateAttributeTypeSelection(PrePostCardAttributeType.email, 0, 1);
        ...
        PrePostCard(
          ...
          prePostUrls: [
            PrePostUrlParams(
              attributes: [
                PrePostCardAttribute(
                    type: PrePostCardAttributeType.userId, queryParam: 'newUserId'),
                PrePostCardAttribute(type: PrePostCardAttributeType.email, queryParam: 'email'),
              ],
            )
          ],
    ...
    test('text is empty', () {
      ...
      expect(
        ...
        PrePostCard(
            ...
            headline: 'headline',
            message: 'message',
            prePostUrls: [PrePostUrlParams(surveyUrl: null)]).toJson(),
      );
      verify(mockView.updateView()).called(1);
    });
  ...
  test('updateEnteredMessage', () {
    final prePostCard = PrePostCard(
        type: PrePostCardType.preEvent, headline: 'headline', message: 'message', prePostUrls: []);
    model.prePostCard = prePostCard;
  ...
  test('initProfile', () async {
    model.email = null;
    when(mockHelper.getEmail(mockUserService, mockCloudFunctionsService)).thenAnswer(
      (_) async => 'email',
    );
      ...
      prePostUrls: [
        PrePostUrlParams(
          attributes: [
            PrePostCardAttribute(type: PrePostCardAttributeType.userId, queryParam: 'userId'),
            PrePostCardAttribute(
                type: PrePostCardAttributeType.discussionId, queryParam: 'discussionId'),
            PrePostCardAttribute(type: PrePostCardAttributeType.email, queryParam: 'email'),
          ],
        )
      ],
  ...
  test('getFinalisedUrl', () {
    ...
    expect(presenter.getFinalisedUrl(PrePostUrlParams(surveyUrl: 'url')), 'url');
  });
