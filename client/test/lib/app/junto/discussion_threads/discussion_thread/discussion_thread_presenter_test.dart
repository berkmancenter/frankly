...
import 'package:junto/app/junto/discussion_threads/discussion_thread/discussion_thread_model.dart';
import 'package:junto/app/junto/discussion_threads/discussion_thread/discussion_thread_presenter.dart';
...
void main() {
  ...
  group('getComments', () {
    test('only top level comments', () {
      ...
      expect(result[0].toJson(), DiscussionThreadCommentUI(comment1, []).toJson());
      expect(result[1].toJson(), DiscussionThreadCommentUI(comment2, []).toJson());
      expect(result[2].toJson(), DiscussionThreadCommentUI(comment3, []).toJson());
      expect(result[3].toJson(), DiscussionThreadCommentUI(comment4, []).toJson());
    });
  ...
  test('updateDiscussionEmotion', () async {
    ...
    final existingEmotion = Emotion(creatorId: 'creatorId', emotionType: EmotionType.heart);
    ...
    await presenter.updateDiscussionEmotion(EmotionType.exclamation, mockDiscussionThread);

    verify(mockFirestoreDiscussionThreadsService.updateEmotion(
  ...
  test('updateDiscussionCommentEmotion', () async {
    ...
    verify(mockFirestoreDiscussionThreadCommentsService.updateEmotion(
      Emotion(creatorId: 'userId', emotionType: EmotionType.exclamation),
      existingEmotion: existingEmotion,
      juntoId: 'juntoId',
      discussionThreadId: 'discussionThreadId',
  ...
  test('getDiscussionThreadStream', () {
    ...
    when(mockJuntoProvider.juntoId).thenReturn('juntoId');
    when(mockFirestoreDiscussionThreadsService.getDiscussionThreadStream(
      juntoId: 'juntoId',
  ...
  test('deleteThread', () async {
    ...
    verify(mockView.showMessage('Post was deleted', toastType: ToastType.success)).called(1);
  });
  ...
  test('addNewComment', () async {
    ...
    verify(mockFirestoreDiscussionThreadCommentsService.addNewDiscussionThreadComment(
        juntoId: 'juntoId',
        discussionThreadId: 'id',
  ...
  test('getDiscussionThreadCommentsStream', () {
    ...
    when(mockJuntoProvider.juntoId).thenReturn('juntoId');
    when(mockFirestoreDiscussionThreadCommentsService.getDiscussionThreadCommentsStream(
            juntoId: 'juntoId', discussionThreadId: 'id'))
        .thenAnswer((_) => stream);
  ...
  test('deleteComment', () async {
    ...
    verify(mockFirestoreDiscussionThreadCommentsService.deleteDiscussionThreadComment(
            juntoId: 'juntoId',
            discussionThreadId: 'id',
            discussionThreadComment: mockDiscussionThreadComment))
        .called(1);
    verify(mockView.showMessage('Comment was deleted', toastType: ToastType.success));
  });

  test('getCommentCount', () {
    final comments = <DiscussionThreadComment>[
      DiscussionThreadComment(
          id: 'id', creatorId: 'creatorId1', comment: 'comment1', isDeleted: true),
      DiscussionThreadComment(
          id: 'id2', creatorId: 'creatorId2', comment: 'comment2', isDeleted: false),
      DiscussionThreadComment(
          id: 'id3', creatorId: 'creatorId3', comment: 'comment3', isDeleted: true),
      DiscussionThreadComment(
          id: 'id4', creatorId: 'creatorId4', comment: 'comment4', isDeleted: false),
    ];
  ...
  test('getCurrentlySelectedEmotion', () {
    when(mockUserService.currentUserId).thenReturn('userId');
    final isSignedIn = Random().nextBool();
    final emotion = Emotion(creatorId: 'userId', emotionType: EmotionType.hundred);
    when(mockUserService.isSignedIn).thenReturn(isSignedIn);
    when(mockEmotionHelper.getMyEmotion([], isSignedIn, 'userId')).thenReturn(emotion);
