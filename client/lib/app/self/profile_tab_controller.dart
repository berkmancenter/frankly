...
class ProfileTabController extends ChangeNotifier {
  ...
  void initialize() {
    ...
    if (juntoId != null) {
      _membershipStream = wrapInBehaviorSubject(
          firestoreMembershipService.getMembershipForUser(juntoId: juntoId!, userId: userId));
    }
    _initChangeRecord();
  }
  ...
  Future<void> _initChangeRecord() async {
    ...
    _allowedSocialPlatforms = allowedSocialPlatforms.map((e) => _replaceIfItemExist(e)).toList();
  }

  SocialMediaItem _replaceIfItemExist(SocialMediaItem item) {
    final result = _changeRecord.socialMediaItems
        .firstWhere((e) => e.socialMediaKey == item.socialMediaKey, orElse: () => item);
    return result;
  }
  ...
  void onEditSocialMedia({required String value, required SocialMediaItem platform}) {
  ...
  Future<void> updateImage(String img) async {
    await userService.updateCurrentUserInfo(_changeRecord.copyWith(imageUrl: img), ['imageUrl']);
