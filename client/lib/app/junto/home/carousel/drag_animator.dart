              ...
              onHorizontalDragUpdate: (details) {
                if (_isDragging) {
                  widget.animationController.value =
                      -(_currentDx(details) / widget.animationControllerValueDivisor)
                          .clamp(-1.0, 1.0);

                  if ((widget.dragBackLocked?.call() ?? false) && _currentDx(details) > 0) {
                    // drag back not allowed, clamp to 0
                    widget.animationController.value = 0;
                    return;
                  }

                  if ((widget.dragForwardLocked?.call() ?? false) && _currentDx(details) < 0) {
