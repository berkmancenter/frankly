...
import 'package:junto/common_widgets/upgrade_tooltip.dart';
...
class _MembersTabState extends State<MembersTab> {
  ...
  int _sortMemberships(Membership a, Membership b) {
    ...
    final aIndex = aStatus == null ? -1 : _membershipOrder.indexOf(aStatus);
    final bIndex = bStatus == null ? -1 : _membershipOrder.indexOf(bStatus);
  ...
  List<Membership> _filterMemberships(List<Membership> memberships) {
    ...
    for (final member in memberships) {
      ...
      final nameMatch = userInfo.info?.displayName?.toLowerCase().contains(search) ?? false;
      final emailMatch = userAdminDetails
              .getInfo(juntoId: JuntoProvider.read(context).juntoId)
              ?.email
              ?.toLowerCase()
              .contains(search) ??
          false;
  ...
  @override
  void initState() {
    final juntoId = Provider.of<JuntoProvider>(context, listen: false).junto.id;
    _memberships = firestoreMembershipService.juntoMembershipsStream(juntoId: juntoId).stream.map(
        (memberships) => memberships.where((m) => m.isAttendee).toList()..sort(_sortMemberships));

    _requests = firestoreJuntoJoinRequestsService.getRequestsForJuntoId(juntoId: juntoId);
  ...
  Future<void> _loadAllUserInfoDetails(List<Membership> memberships) async {
    ...
    for (final batch in batches) {
      await Future.wait(batch.map((m) => UserInfoProvider.forUser(m.userId).infoFuture));
          ...
          _loadUsersFuture ??= Future.wait([
            ...
            ...memberships.map(
              (m) => UserAdminDetailsProvider.forUser(m.userId)
                  .getInfoFuture(juntoId: JuntoProvider.read(context).juntoId),
            ),
          ]);
        });
      },
  ...
  Future<void> _resolveRequest({
    ...
    await cloudFunctionsService.resolveJoinRequest(ResolveJoinRequestRequest(
  ...
  Future<void> _downloadMembersData(List<Membership> membershipList) async {
    ...
    if (membersList.isNotEmpty) {
      await alertOnError(context, () async {
        final members =
            await userService.getMemberDetails(membersList: membersList, juntoId: juntoId);
        if (members.isNotEmpty) {
          ...
          final fileName = 'members-data-$juntoId.csv';
        ...
        children: [
          ...
          UserAdminDetailsBuilder(
            ...
            builder: (_, loading, detailsSnapshot) {
              ...
              final isError = detailsSnapshot.hasError || email == null || email.isEmpty;
          ...
          Row(
            ...
            mainAxisSize:
                !responsiveLayoutService.isMobile(context) ? MainAxisSize.min : MainAxisSize.max,
            children: [
              Padding(
                ...
                child: ActionButton(
                  ...
                  onPressed: () =>
                      alertOnError(context, () => _resolveRequest(request: request, approve: true)),
                ),
              ),
              Padding(
                padding: EdgeInsets.symmetric(horizontal: 4),
                child: ActionButton(
                  ...
                  onPressed: () => alertOnError(
                      context, () => _resolveRequest(request: request, approve: false)),
                ),
              ),
            ],
          ),
  ...
  Widget _buildRequestList(List<MembershipRequest> requestList) {
    return ConstrainedBox(
        ...
        child: ListView(
          shrinkWrap: true,
          children: [
            for (var i = 0; i < requestList.length; i++) _buildRequestEntry(i, requestList[i]),
          ],
        ));
  }

  Widget _buildRequestsSection() {
    return JuntoStreamBuilder<List<MembershipRequest>>(
  ...
  Widget _buildMembersSection() {
    return JuntoStreamBuilder<List<Membership>>(
      ...
      builder: (context, allMembershipDocs) => JuntoStreamGetterBuilder<PlanCapabilityList>(
        streamGetter: () => cloudFunctionsService
            .getJuntoCapabilities(
              GetJuntoCapabilitiesRequest(juntoId: context.read<JuntoProvider>().juntoId),
            )
            .asStream(),
        entryFrom: '_MembersTabState._buildMembersSection_2',
        builder: (context, caps) {
          final membershipList =
              allMembershipDocs?.where((element) => !(element.invisible)).toList();

          final adminCapabilityCount = caps?.adminCount ?? 0;
          // Count mods + admins in total allowed admin count
          final adminCount = membershipList?.where((element) => element.isMod).length;
          final allowPromoteToAdmin = adminCount != null && adminCount < adminCapabilityCount;

          final facilitatorCapabilityCount = caps?.facilitatorCount ?? 0;
          final facilitatorCount = membershipList
              ?.where((element) => element.status == MembershipStatus.facilitator)
              .length;
          final allowPromoteToFacilitator =
              facilitatorCount != null && facilitatorCount < facilitatorCapabilityCount;
                ...
                children: [
                  ...
                  ConstrainedBox(
                    constraints: BoxConstraints(
                        maxHeight: 400,
                        maxWidth: !responsiveLayoutService.isMobile(context) ? 600 : 400),
                    child: AnimatedBuilder(
                      animation: Listenable.merge(
                        [
                          ...membershipList.map((m) => UserInfoProvider.forUser(m.userId)),
                          ...membershipList.map((m) => UserAdminDetailsProvider.forUser(m.userId)),
                        ],
                      ),
                      builder: (_, __) {
                        final filteredMembers = _filterMemberships(membershipList);
                        if (filteredMembers.isEmpty && !isNullOrEmpty(_currentSearch)) {
                          return Padding(
  ...
  Widget build(BuildContext context) {
    final requiresApproval = context.read<JuntoProvider>().settings.requireApprovalToJoin;
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        SizedBox(height: 8),
        JuntoStreamBuilder<List<Membership>>(
          stream: _memberships,
          entryFrom: '_MembersTabState._build',
          errorMessage: 'Something went wrong loading memberships. Please refresh.',
          builder: (context, membershipList) => _buildSearchBar(membershipList ?? []),
        ),
        SizedBox(height: 20),
  ...
  JuntoListView _buildPermissionMessage() {
...
class RolePermissionListTile extends StatelessWidget {
  ...
  const RolePermissionListTile(
      {Key? key, required this.title, required this.permissions, required this.icon})
      : super(key: key);
...
class ChangeMembershipDropdown extends StatefulWidget {
  ...
  @override
  _ChangeMembershipDropdownState createState() => _ChangeMembershipDropdownState();
}
...
class _ChangeMembershipDropdownState extends State<ChangeMembershipDropdown> {
  ...
  Future<void> _updateMembership(MembershipStatus? newStatus) async {
    if ((newStatus?.isMod ?? false) && !widget.allowPromoteToAdmin) {
      ...
      return;
    } else if (newStatus == MembershipStatus.facilitator && !widget.allowPromoteToFacilitator) {
      setState(() => _showUpgradeTooltip = true);
      return;
    }

    if (newStatus == MembershipStatus.nonmember) {
      final delete = await ConfirmDialog(mainText: 'Are you sure you want to remove member?')
        ...
        Flexible(
          child: Container(
            ...
            child: UpgradeTooltip(
              isTooltipVisible: kShowStripeFeatures ? _showUpgradeTooltip : false,
              isBelowIcon: false,
              ...
              child: DropdownButton<MembershipStatus>(
                ...
                selectedItemBuilder: (BuildContext context) => [
                  if (widget.membership.status == MembershipStatus.owner) MembershipStatus.owner,
                  MembershipStatus.admin,
                  MembershipStatus.mod,
                  MembershipStatus.facilitator,
                  MembershipStatus.member,
                ...
                items: [
                  ...
                  (value) {
                    ...
                    final isUnallowedAdminPromotion = !widget.allowPromoteToAdmin && value.isMod;
                    final isUnallowedFacilitatorPromotion =
                        !widget.allowPromoteToFacilitator && value == MembershipStatus.facilitator;
                    final isDisabled = !isCurrentStatus &&
                        (isUnallowedAdminPromotion || isUnallowedFacilitatorPromotion);
                    final textStyle = AppTextStyle.body.copyWith(
                        color: isDisabled ? AppColor.gray1.withOpacity(.5) : AppColor.gray1);
                    return DropdownMenuItem<MembershipStatus>(
                      value: value,
                      child: Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          isDisabled ? UpgradeIcon(isDisabledColor: true) : value.icon,
                          SizedBox(width: 5),
                          Flexible(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
