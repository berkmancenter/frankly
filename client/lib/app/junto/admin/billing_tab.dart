...
import 'package:junto/common_widgets/confirm_dialog_white.dart';
...
class _AdminBillingTabState extends State<AdminBillingTab> {
  ...
  Widget _buildBillingContainer() => Container(
        ...
        child: JuntoStreamGetterBuilder<BillingSubscription?>(
          streamGetter: () => firestoreBillingSubscriptionsService.getActiveSubscription(
            userId: userService.currentUserId!,
            communityId: context.watch<JuntoProvider>().junto.id,
          ),
          builder: (context, subscription) {
            return _AdminBillingContainerContent(
                ...
                key: Key(
                    subscription?.type ?? _AdminBillingContainerContentState._freeSubscriptionType),
...
class _AdminBillingContainerContent extends StatefulWidget {
  ...
  @override
  _AdminBillingContainerContentState createState() => _AdminBillingContainerContentState();
}

class _AdminBillingContainerContentState extends State<_AdminBillingContainerContent> {
  ...
  late final String? _initialPlan = widget.subscription?.type ?? _freeSubscriptionType;

  late Plan _selectedPlan =
      plans.firstWhere((element) => element.typeCode == _initialPlan, orElse: () => plans.first);

  bool get _newPlanSelected => _selectedPlan.typeCode != _initialPlan;

  Future<void> _updatePlanPressed() async {
    final junto = context.read<JuntoProvider>().junto;
    final selectedPlanType = _selectedPlan.planType;

    final hasSubscription =
        await firestoreBillingSubscriptionsService.userHasResumableSubscriptionForCommunity(
      userId: userService.currentUserId!,
      communityId: junto.id,
    );

    if (_selectedPlan.typeCode == _customSubscriptionType) {
      await launch('https://frankly.org/pricing');
    } else if (hasSubscription) {
      if (selectedPlanType != null) {
        final priceInfo = await cloudFunctionsService.getStripeSubscriptionPlanInfo(
        ...
        final priceString =
            (centsPart == 0) ? '\$$dollarPart' : formatter.format(priceInfo.priceInCents / 100.0);

        final confirm = await ConfirmDialogWhite(
          title: 'Update to ${priceInfo.name}?',
          mainText: 'Effective immediately you will be enrolled in the ${priceInfo.name} '
        ...
        if (confirm) {
          await cloudFunctionsService
              .updateStripeSubscriptionPlan(UpdateStripeSubscriptionPlanRequest(
            juntoId: junto.id,
        ...
        if (_selectedPlan.typeCode == _freeSubscriptionType) {
          final confirm = await ConfirmDialogWhite(
            ...
            mainText: 'Immediately cancel your current plan and enroll in the free plan?',
          ...
          if (confirm) {
            await cloudFunctionsService
                .cancelStripeSubscriptionPlan(CancelStripeSubscriptionPlanRequest(
              juntoId: junto.id,
            ));
          }
        }
      ...
      if (selectedPlanType != null) {
        final response = await cloudFunctionsService
            .createSubscriptionCheckoutSession(CreateSubscriptionCheckoutSessionRequest(
          type: selectedPlanType,
          appliedJuntoId: junto.id,
          returnRedirectPath: 'space/${junto.displayId}/admin?tab=billing',
        ));
        GetIt.instance<StripeClientService>().redirectToCheckout(sessionId: response.sessionId);
      }
    }
  }
  ...
  @override
  Widget build(BuildContext context) {
    final isUserBillingManager = context.watch<CommunityPermissionsProvider>().isUserBillingManager;
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        if (isUserBillingManager) _buildTitle(widget.subscription?.type ?? _freeSubscriptionType),
        SizedBox(height: 20),
        if (isUserBillingManager) _buildUserIsBillingManager() else _buildUserNotBillingManager(),
        SizedBox(height: 20),
          ...
          Wrap(
            children: [
              ...
              ActionButton(
                ...
                onPressed:
                    _newPlanSelected ? () => alertOnError(context, _updatePlanPressed) : null,
              ),
            ],
          ),
      ],
  ...
  Widget _buildTitle(String type) {
    ...
    return JuntoText(
      '${junto.name ?? 'This Community'} is on the $planType Plan',
      style: AppTextStyle.subhead.copyWith(fontSize: 22, color: AppColor.darkBlue),
    );
  }
  ...
  Widget _buildPlansDropdown() => Container(
        ...
        child: DropdownButton<Plan>(
          ...
          borderRadius: BorderRadius.circular(10),
          value: _selectedPlan,
          onChanged: (val) => val != null ? setState(() => _selectedPlan = val) : null,
  ...
  Widget _buildUserNotBillingManager() {
    ...
    return Column(
      ...
      children: [
        ...
        UserInfoBuilder(
          userId: context.watch<JuntoProvider>().junto.creatorId,
        ...
        return Wrap(
          ...
          children: [
            ...
            ActionButton(
              ...
              onPressed: () async {
                final junto = context.read<JuntoProvider>().junto;
                final response = await cloudFunctionsService.getStripeBillingPortalLink(
                  GetStripeBillingPortalLinkRequest(
                    responsePath: 'space/${junto.displayId}/admin?tab=billing',
                  ),
                );
