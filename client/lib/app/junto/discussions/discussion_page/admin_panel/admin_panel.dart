...
import 'package:junto/app/junto/discussions/discussion_page/admin_panel/breakout_rooms_dialog.dart';
import 'package:junto/app/junto/discussions/discussion_page/admin_panel/jump_to_room_dialog.dart';
import 'package:junto/app/junto/discussions/discussion_page/admin_panel/kick_dialog.dart';
import 'package:junto/app/junto/discussions/discussion_page/admin_panel/reassign_breakout_room_dialog.dart';
...
import 'package:junto/app/junto/discussions/discussion_page/live_meeting/video/conference/fake_participants_dialog.dart';
...
class _AdminPanelState extends State<AdminPanel> {
  LiveMeetingProvider get _provider => Provider.of<LiveMeetingProvider>(context);
  LiveMeetingProvider get _providerRead => Provider.of<LiveMeetingProvider>(context, listen: false);
  ...
  Widget _buildMeetingProviderParticipantEntry(MeetingProviderParticipant participant) {
      ...
      Container(
        ...
        child: Row(
          children: [
            ...
            if (_discussionProvider.discussion.discussionType != DiscussionType.hostless) ...[
              SizedBox(width: 4),
              ActionButton(
  ...
  Widget _buildPaginatedParticipants() {
    return FirestoreListView(
      ...
      query: firestoreDiscussionService.discussionParticipantsQuery(
        discussion: DiscussionProvider.watch(context).discussion,
      ),
      pageSize: 40,
      emptyBuilder: (_) => JuntoText('No one is here yet.'),
      errorBuilder: (_, __, ___) =>
          JuntoText('Something went wrong loading participants. Please refreh.'),
    );
  }

  List<Widget> _buildDefaultParticipantList() {
    var participantSuffix = '';
    final useMeetingProviderParticipants = _discussionProvider.discussion.isHosted;
    final participantCount = (useMeetingProviderParticipants
            ? _provider.meetingProviderParticipants?.length
            : _discussionProvider.participantCount) ??
        0;
...
class _BreakoutRoomGridState extends State<BreakoutRoomGrid> {
  ...
  Widget _buildBreakoutRoomFilterToggle() {
    ...
    return JuntoInkWell(
      ...
      onTap: () => setState(() => _showOnlyAlertedRooms = !_showOnlyAlertedRooms),
      child: Row(
        children: [
          Expanded(
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                color:
                    _showOnlyAlertedRooms ? AppColor.black.withOpacity(0.4) : AppColor.brightGreen,
              ),
              alignment: Alignment.center,
              child: JuntoText(
                'All rooms',
                style: TextStyle(
                  color: _showOnlyAlertedRooms ? AppColor.white : Theme.of(context).primaryColor,
                ),
              ),
            ),
          ),
  ...
  Widget _buildAllBreakoutsPaginated(BoxConstraints constraints, Set<String> needsHelpRoomIds) {
        ...
        return GridView.builder(
          ...
          itemBuilder: (_, index) {
            if (snapshot.hasError) {
              return JuntoText('Something went wrong loading breakout rooms. Please refresh');
            } else if (snapshot.docs.isEmpty) {
              return JuntoText('No rooms found.');
            }
            ...
            final room = BreakoutRoom.fromJson(
                fromFirestoreJson(snapshot.docs[index].data() as Map<String, dynamic>));
  ...
  Widget _buildNeedsHelpGrid(BoxConstraints constraints, List<BreakoutRoom> rooms) {
    if (rooms.isEmpty) {
      return JuntoText('No rooms need help.');
    }
    return GridView.builder(
  ...
  Widget build(BuildContext context) {
    ...
    return JuntoStreamBuilder<BreakoutRoom?>(
      entryFrom: '_BreakoutRoomGridState.build',
      stream: _waitingRoomStream ??= firestoreLiveMeetingService.breakoutRoomStream(
        discussion: DiscussionProvider.read(context).discussion,
        breakoutRoomSessionId: breakoutSessionId,
        breakoutRoomId: breakoutsWaitingRoomId,
      ),
      builder: (_, waitingRoom) => JuntoStreamBuilder<List<Participant>>(
        entryFrom: '_BreakoutRoomGridState.build',
        ...
        builder: (context, waitingRoomParticipants) => JuntoStreamBuilder<List<BreakoutRoom>>(
          entryFrom: '_BreakoutRoomGridState.build',
          stream: _needHelpRooms ??= firestoreLiveMeetingService.breakoutRoomsStream(
            discussion: DiscussionProvider.watch(context).discussion,
            breakoutRoomSessionId: breakoutSessionId,
          ...
          builder: (context, needHelpRooms) {
            ...
            final usersAssignedToWaitingRoom = waitingRoom?.participantIds.toSet() ?? {};
            final waitingRoomIsActive =
                waitingRoomParticipants?.any((p) => usersAssignedToWaitingRoom.contains(p.id)) ??
                    false;
            final waitingRoomAlreadyFlaggedAsNeedHelp =
                needHelpRooms?.any((r) => r.roomId == breakoutsWaitingRoomId) ?? false;
            final List<BreakoutRoom> allNeedHelpRooms = [
              if (waitingRoomIsActive && !waitingRoomAlreadyFlaggedAsNeedHelp) waitingRoom!,
              ...needHelpRooms!,
            ];
            return Column(
              children: [
                    ...
                    ActionButton(
                      onPressed: () => alertOnError(context, () async {
                        ...
                        if (!isNullOrEmpty(roomNumber)) {
                          final room =
                              await firestoreLiveMeetingService.getBreakoutRoomFromRoomNumber(
                            discussion: DiscussionProvider.read(context).discussion,
                          ...
                          if (room == null) {
                            throw VisibleException('Room $roomNumber not found.');
                          }
                ...
                SizedBox(height: 16),
                if (!isNullOrEmpty(liveMeetingProvider.currentBreakoutRoomId)) ...[
                  ActionButton(
                    onPressed: () =>
                        setState(() => _selectedRoom = liveMeetingProvider.currentBreakoutRoomId),
                  ...
                  child: LayoutBuilder(
                    builder: (context, constraints) => _showOnlyAlertedRooms
                        ...
                        : _buildAllBreakoutsPaginated(
                            constraints, allNeedHelpRooms.map((r) => r.roomId).toSet()),
                  ),
                ),
              ],
            );
...
class _MeetingControlsMenuState extends State<_MeetingControlsMenu> {
  ...
  List<PopupMenuEntry<Function()>> _getMoreMenuItems() {
    ...
    final isLocked = provider.discussion.isLocked;
    return [
          ...
          value: () async {
            await alertOnError(
                ...
                () => firestoreDiscussionService.updateDiscussion(
                      discussion: provider.discussion.copyWith(isLocked: !isLocked),
          ...
          value: () => alertOnError(context, () async {
            final numParticipants = await FakeParticipantsDialog(
              fakeParticipantCount:
                  LiveMeetingProvider.read(context).conferenceRoom?.numFakeParticipants ?? 0,
            ).show();
            if (numParticipants == null) return;
            final countValue = int.tryParse(numParticipants);
            if (countValue == null) return;

            LiveMeetingProvider.read(context).conferenceRoom?.numFakeParticipants = countValue;
          }),
          child: JuntoText(
...
class __ParticipantMenuState extends State<_ParticipantMenu> {
      ...
      PopupMenuItem<Function()>(
        value: () => alertOnError(context, () async {
          KickDialogResult? confirmResult = await KickDialog(
            userName: UserInfoProvider.forUser(widget.kickedUserId).info?.displayName,
          ).show();
          ...
          await swallowErrors(
            () => cloudFunctionsService.kickParticipant(
          ...
          await firestoreDiscussionService.kickParticipant(
  ...
  Widget build(BuildContext context) {
    ...
    return Semantics(label:'Participant Actions', child: JuntoInkWell(
      hoverColor: AppColor.black.withOpacity(0.1),
      onTap: () => _showMoreMenu(menuItems),
      child: Icon(
        Icons.more_vert,
...
class _BreakoutRoomButtonState extends State<BreakoutRoomButton> {
  ...
  void initState() {
    super.initState();
    _breakoutParticipantsStream = firestoreLiveMeetingService.breakoutRoomParticipantsStream(
      discussion: DiscussionProvider.read(context).discussion,
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<Participant>>(
      ...
      builder: (context, participantsSnapshot) {
        var participants = participantsSnapshot.data?.map((p) => p.id).toList() ?? [];
        if (isWaitingRoom) {
          final assignedHere = widget.room.participantIds.toSet();
          participants = participants.where((id) => assignedHere.contains(id)).toList();
        }
        final participantCount = participants.length;

        final room = widget.room;
        final roomDisplayName =
            room.roomId == breakoutsWaitingRoomId ? room.roomName : 'Room ${room.roomName}';
        ...
        final isCurrentRoom = liveMeetingProvider.currentBreakoutRoomId == room.roomId &&
              ...
              children: [
                JuntoText(
                  ...
                  style: TextStyle(
                    ...
                    color: isCurrentRoom ? Theme.of(context).primaryColor : AppColor.white,
                  ),
                ),
                SizedBox(height: 10),
                Container(
                      ...
                      if (participantsSnapshot.connectionState == ConnectionState.waiting)
                        ...
                        JuntoText(
                          ...
                          style: TextStyle(
                            ...
                            color: needsHelp
                                ? AppColor.darkRed
                                : (isCurrentRoom ? Theme.of(context).primaryColor : AppColor.white),
                          ),
                        ),
                    ],
                  ),
...
class _BreakoutRoomDetailsState extends State<BreakoutRoomDetails> {
      ...
      child: Row(
        children: [
          ...
          ActionButton(
            ...
            onPressed: () => alertOnError(context, () async {
              final ReassignResult? newRoomAssignment = await ReassignBreakoutRoomDialog(
                outerContext: context,
                userId: id,
                currentRoomNumber: [
              ...
              final newBreakoutRoom = await Provider.of<LiveMeetingProvider>(context, listen: false)
              ...
              if (reassignId == reassignNewRoomId &&
                  newRoomAssignment?.expectedNewRoom?.toString() != newBreakoutRoom.roomName) {
                await ConfirmDialog(
  ...
  Widget build(BuildContext context) {
    return JuntoStreamBuilder<BreakoutRoom?>(
      ...
      builder: (context, room) => JuntoStreamBuilder<List<Participant>>(
        entryFrom: '_BreakoutRoomDetailsState.build',
        stream: _breakoutParticipantsStream,
        builder: (context, liveBreakoutParticipants) {
          ...
          var participantIds = localLiveBreakoutParticipants.map((p) => p.id).toList();
          if (isWaitingRoom) {
            final assignedHere = room.participantIds.toSet();
            participantIds = participantIds.where((id) => assignedHere.contains(id)).toList();
          }
          ...
          final needsHelp = localRoom.flagStatus == BreakoutRoomFlagStatus.needsHelp ||
              (localRoom.roomId == breakoutsWaitingRoomId && participantCount > 0);
          ...
          return Column(
            children: [
              JuntoInkWell(
                onTap: widget.goBack,
                child: Container(
                  color: needsHelp ? AppColor.darkRed : AppColor.black.withOpacity(0.4),
                  padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
                  child: Row(
                    children: [
                      ...
                      if (provider.currentBreakoutRoomId == localRoom.roomId &&
                        ...
                        ActionButton(
                          ...
                          onPressed: () async {
                            final reassignUser =
                                provider.currentBreakoutRoomId == provider.assignedBreakoutRoomId;
                        ...
                        ActionButton(
                          onPressed: () {
                            provider.enterBreakoutRoom(roomId: localRoom.roomId);
                          },
                          color: needsHelp ? AppColor.redDarkMode : Colors.transparent,
                          textColor: needsHelp ? AppColor.white : AppColor.brightGreen,
                          overlayColor: AppColor.white.withOpacity(0.3),
                          text: 'Enter Room',
                        ),
                    ],
              ...
              if (localRoom.flagStatus == BreakoutRoomFlagStatus.needsHelp) ...[
                ...
                ThickOutlineButton(
                  onPressed: () async {
                    ...
                    await alertOnError(
                        context,
                        () => cloudFunctionsService.updateBreakoutRoomFlagStatus(
                              request: UpdateBreakoutRoomFlagStatusRequest(
                                discussionPath: provider.discussionPath,
                                breakoutSessionId: provider.liveMeeting?.currentBreakoutSession
                                        ?.breakoutRoomSessionId ??
                                    '',
                                roomId: roomId,
                                flagStatus: BreakoutRoomFlagStatus.unflagged,
              ...
              if (participantCount == 0)
                Padding(
                  ...
                  child: JuntoText('No one is here yet'),
                )
              else
                Expanded(
                  child: ListView(
                    children: [
                      for (final participantId in participantIds)
                        _buildBreakoutRoomParticipant(participantId, room, context),
                    ],
                  ),
                ),
            ],
