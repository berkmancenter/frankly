...
import 'package:junto/app/junto/discussions/discussion_page/av_check/av_check_provider.dart';
...
class _AvCheckPage extends StatefulWidget {
  _AvCheckPage({Key? key}) : super(key: key);
...
class _AvCheckPageState extends State<_AvCheckPage> {
    ...
    return Container(
      ...
      child: UserInfoBuilder(
        ...
        builder: (context, loading, user) {
          ...
          final userImage = data.imageUrl ?? generateRandomImageUrl(seed: data.hashCode);
          return Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              JuntoText('Hi${name != null ? ' $name' : ''}, ready to join?',
  ...
  Widget _buildDiagnoseIssuesButton() => JuntoUiMigration(
  ...
  Widget _buildAVIcons() => Row(
        ...
        children: [
          _buildAvIcon(
            ...
            asset: provider.cameraOn ? AppAsset.kCameraPng : AppAsset.kCameraOffPng,
          ),
          SizedBox(width: 10),
          _buildAvIcon(
            onTap: provider.toggleMic,
            asset: provider.micOn ? AppAsset.kAudioOnPngWhite : AppAsset.kAudioOffPngWhite,
          ),
        ],
      );
  ...
  Widget _buildDevicesDropdown({
    ...
    final devices = provider.devicesList?.where((d) => d.kind == deviceKind).toList() ?? [];
            ...
            builder: (context) {
              if (devices.isEmpty) {
                ...
                return DropdownButton<String>(
                  underline: SizedBox(),
                  onChanged: (deviceId) {
                    final id = devices.firstWhere((d) => d.deviceId == deviceId).deviceId;
                    if (id != null) {
                      onChanged(id);
                    }
                  },
                  ...
                  selectedItemBuilder: (context) => devices.map(_buildDropdownItem).toList(),
                  items: [
                    for (final device in devices)
                      DropdownMenuItem(
                        value: device.deviceId,
                        child: _buildDropdownItem(device, textColor: AppColor.black),
                      ),
                  ],
                );
              }
