...
class NetworkingStatusPresenter {
  ...
  void updateNetworkQuality() {
    ...
    const badStates = [QualityType.qualityPoor, QualityType.qualityVbad, QualityType.qualityBad, QualityType.qualityDown ];
    if (badStates.contains(_model.networkQualityLevel) &&
        isVideoEnabled) {
      final Timer? timer = _model.timer;
      loggingService.log('NetworkingStatusPresenter.updateNetworkQuality: Bad network connection');

      // Only spawn new timer if it's not initialised (first time) or active already
      if (timer == null || !timer.isActive) {
        loggingService.log('NetworkingStatusPresenter.updateNetworkQuality: Spawning new timer');
        ...
        _model.timer = Timer.periodic(_kPrimaryThresholdDuration, (timer) {
          if (badStates.contains(_model.networkQualityLevel)) {
            ...
            loggingService
                .log('NetworkingStatusPresenter.updateNetworkQuality: Connection improved');
            timer.cancel();
          }
        });
      }
