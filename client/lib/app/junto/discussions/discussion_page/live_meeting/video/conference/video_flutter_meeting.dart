...
import 'package:junto/app/junto/discussions/discussion_page/live_meeting/meeting_guide/meeting_guide_minimized_card/meeting_guide_minimized_card.dart';
...
import 'package:junto/app/junto/discussions/discussion_page/live_meeting/video/conference/need_help_dialog.dart';
...
import 'package:junto_models/firestore/discussion.dart' as discussion;
...
class JuntoGlobalKey extends LabeledGlobalKey {
  static final Map<String, JuntoGlobalKey> _participantKeys = {};

  final String distinctLabel;

  JuntoGlobalKey._(this.distinctLabel) : super(distinctLabel);

  factory JuntoGlobalKey.fromLabel(String label) =>
      _participantKeys[label] ??= JuntoGlobalKey._(label);
}
...
class _VideoFlutterMeetingState extends State<VideoFlutterMeeting> {
  ...
  ConferenceRoom get _conferenceRoomRead => Provider.of<ConferenceRoom>(context, listen: false);
  ...
  Future<void> _connectToRoom() async {
    _onConferenceRoomException = _conferenceRoomRead.onException.listen((err) async {
  ...
  JuntoGlobalKey _getGlobalKey(String label) {
    return JuntoGlobalKey.fromLabel(label);
  }
  ...
  Widget _buildVideoLayout() {
    ...
    return Stack(
      children: [
        ...
        if (DiscussionProvider.watch(context).discussion.discussionSettings?.alwaysRecord == true)
          Container(
  ...
  Widget _buildMainVideoContent(BuildContext context, ConferenceRoom conferenceRoom) {
    ...
    if (screenSharer != null) {
      return Column(
        children: [
          ...
          Expanded(
            child: _SidePanelParticipants(remainingParticipants: conferenceRoom.participants),
          ),
        ],
      );
    } else {
    ...
    return GlobalKeyedSubtree(
      ...
      child: MeetingGuideCard(
        onMinimizeCard: () => alertOnError(
          context,
          () => LiveMeetingProvider.read(context).updateGuideCardIsMinimized(isMinimized: true),
        ),
      ),
    );
  }
  ...
  Widget _buildLayoutViewButtons() {
    final additionalButtonsOffset =
        DiscussionProvider.watch(context).discussion.discussionSettings?.alwaysRecord ?? false
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          if (liveMeetingProvider.isMeetingCardMinimized && liveMeetingProvider.showGuideCard)
  ...
  Widget _buildViewIconButton(String message, IconData iconData, LiveMeetingViewType type) {
    return Tooltip(
      message: message,
      child: Padding(
  ...
  Widget _buildHostlessDesktop() {
    ...
    final guideCardTakeover = MeetingGuideCardStore.watch(context)?.guideCardTakeover == true &&
        !isUnifyAmerica &&
    ...
    return LayoutBuilder(
      builder: (_, constraints) {
        ...
        var highlightedParticipants = participants.take(maxHighlighted).toList().reversed.toList();
        ...
        _currentStageOrdering = highlightedParticipants.map((p) => p.userId).toList();

        final remainingParticipants = participants.skip(maxHighlighted).toList();

        return Column(
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            if (!isUnifyAmerica) _buildLayoutViewButtons(),
            Expanded(
              child: Container(
                ...
                child: Builder(
                  builder: (_) {
                    ...
                    var widgets = [
                      ...participantWidgets,
                      if (showGuideCard && !liveMeetingProvider.isMeetingCardMinimized)
                        ConstrainedBox(
            ...
            if (remainingParticipants.isNotEmpty)
              SizedBox(
                height: 160,
                child: _SidePanelParticipants(remainingParticipants: remainingParticipants),
              ),
          ],
        );
      },
  ...
  Widget _buildBradyBunchViewWidget() {
    final showGuideCard = liveMeetingProvider.showGuideCard;
    final showGuideCardLayout = showGuideCard && !liveMeetingProvider.isMeetingCardMinimized;
...
class GetHelpButton extends StatefulWidget {
  ...
  static Future<void> getHelp(BuildContext context) async {
    final unifyAmericaController = UnifyAmericaController.read(context);
    final liveMeetingProvider = LiveMeetingProvider.read(context);
    final discussionProvider = DiscussionProvider.read(context);
    final juntoProvider = JuntoProvider.read(context);

    analytics.logEvent(AnalyticsPressDiscussionHelpEvent(
    ...
    final isUnifyChallenge = unifyAmericaController != null;
    final isHostless =
        discussionProvider.discussion.discussionType == discussion.DiscussionType.hostless;
    final isInLiveStreamBreakouts =
        isHostless || (liveMeetingProvider.isInBreakout && discussionProvider.isLiveStream);
    final showContactAdmin =
        isUnifyChallenge || isInLiveStreamBreakouts || juntoProvider.isAmericaTalks;
    final needHelp = await NeedHelpDialog(
      showContactAdmin: showContactAdmin,
      isUnifyChallenge: isUnifyChallenge,
      isAmericaTalks: juntoProvider.isAmericaTalks,
    ).show();
    ...
    if (isUnifyChallenge) {
      await alertOnError(
        context,
        () async => UnifyAmericaController.read(context)!.onNeedsHelp(),
            ...
            child: Padding(
              ...
              child: JuntoText(
                ...
                style: TextStyle(
                  ...
                  color: Theme.of(context).isDark ? AppColor.brightGreen : AppColor.darkBlue,
                ),
              ),
            ),
          ),
...
class _SidePanelParticipantsState extends State<_SidePanelParticipants> {
  ...
  void didChangeDependencies() {
    ...
    WidgetsBinding.instance?.addPostFrameCallback((_) => _checkUpdateControllerAttached());
  }
            ...
            ListView(
              ...
              children: [
                for (final p in widget.remainingParticipants)
                  ParticipantWidget(
                    globalKey: JuntoGlobalKey.fromLabel(p.userId),
                    participant: p,
                  ),
              ],
            ),
                  ...
                  if (_controller.position.atEdge && _controller.position.pixels == 0)
                    ...
                    JuntoInkWell(
                      onTap: () => _controller.animateTo(
                        math.max(-25, _controller.offset - constraints.maxWidth * 0.75),
