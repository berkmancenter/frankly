...
import 'package:junto/app/junto/discussions/discussion_page/live_meeting/video/debug.dart';
...
import 'package:junto_models/firestore/discussion.dart' hide Participant;
...
class FakeParticipant extends AgoraParticipant {
  ...
  FakeParticipant({required this.id, required bool isLocal})
      : super(rtcEngine: null, agoraUid: id, userId: id.toString(), isLocal: isLocal);
...
class VideoParticipant implements MeetingProviderParticipant {
  ...
  VideoParticipant(this.participant, this.discussion, this.discussionPath);
...
class ConferenceRoom with ChangeNotifier {
  ...
  int get maxHighlightedParticipants {
    return liveMeetingProvider.showGuideCard && !liveMeetingProvider.isMeetingCardMinimized ? 1 : 2;
  }
  ...
  bool get isLocalSharingScreenActive => screenSharerUserId == userService.currentUserId;

  String? get screenSharerUserId =>
      participants.firstWhereOrNull((p) => p.screenshareTrack != null)?.identity;
  ...
  String? get dominantSpeakerSid => _debouncedDominantSpeakerStream?.value?.userId;
  ...
  List<AgoraParticipant> get participants {
    ...
    for (final participant in _orderedParticipants) {
      final existingIndex = participantsCopy.indexWhere((p) => p.userId == participant.userId);
      if (existingIndex >= 0) {
    ...
    localHandRaisedParticipants.sort((a, b) {
      final aHandRaisedTime = meetingGuideCardModel.getHandRaisedTime(a.identity);
      final bHandRaisedTime = meetingGuideCardModel.getHandRaisedTime(b.identity);
    ...
    final handRaisedIds = localHandRaisedParticipants.map((p) => p.userId).toSet();
    ...
    final dominantSpeaker =
        _orderedParticipants.firstWhereOrNull((p) => p.userId == dominantSpeakerSid);
    final dominantSpeakerIndex =
        dominantSpeaker != null ? _orderedParticipants.indexOf(dominantSpeaker) : -1;
    ...
    final pinnedIds = agendaProvider.currentLiveMeeting?.pinnedUserIds.toSet() ?? {};
    final pinnedParticipants =
        _orderedParticipants.where((p) => pinnedIds.contains(p.identity)).toList();
    ...
    return [
      ...
      for (int i = 0; i < _numFakeParticipants; i++) FakeParticipant(id: i, isLocal: false),
    ];
  }
  ...
  void initialize(BuildContext context) {
    ...
    liveMeetingProvider.discussionProvider.addListener(_muteOthersOnOverride);
  }

  void _muteOthersOnOverride() {
    final mutedUsers = liveMeetingProvider.discussionProvider.discussionParticipants
    ...
    for (final participant in _room?.remoteParticipants ?? <AgoraParticipant>[]) {
      final userId = participant.identity;

      if (newlyMutedUsers.contains(userId) || newlyUnmutedUsers.contains(userId)) {
        participant.toggleMuteOverride(isMuted: newlyMutedUsers.contains(userId));
      }
    }
  }
  ...
  Future<void> connect() async {
    ...
    try {
      ...
      _room = AgoraRoom(
        ...
        discussionProvider: liveMeetingProvider.discussionProvider,
        conferenceRoom: this,
      );
      await _room!.connect(
        enableVideo: false,
  ...
  void _disposeStreamsAndSubscriptions() {
    liveMeetingProvider.discussionProvider.removeListener(_muteOthersOnOverride);
  ...
  Future<void> toggleVideoEnabled({
    ...
    if (updatedEnabledValue) {
      ...
      if (!granted) {
        await showAlert(navigatorState.context,
  ...
  Future<void> toggleAudioEnabled({
    ...
    await _audioTogglingLock.synchronized(
      () async {
        if (updatedEnabledValue && liveMeetingProvider.audioTemporarilyDisabled) {
          return;
        }
        ...
        final audioEnableFutures = [
          ...
          if ((liveMeetingProvider.discussionProvider.selfParticipant?.muteOverride ?? false) &&
              updatedEnabledValue)
            firestoreLiveMeetingService.updateParticipantMuteOverride(
              discussion: liveMeetingProvider.discussionProvider.discussion,
  ...
  Future<void> onConnected(AgoraRoom room) async {
    ...
    _debouncedDominantSpeakerStream = BehaviorSubjectWrapper(room.dominantSpeakerStream
        .distinct()
    ...
    _unraiseHandSubscription = _debouncedDominantSpeakerStream!
      ...
      final dismissRaisedHand = room?.localParticipant?.agoraUid == dominantSpeaker?.agoraUid;
      final isHandRaised = meetingGuideCardModel.getHandIsRaised(userService.currentUserId!);
      final currentAgendaModelItemId = meetingGuideCardModel.meetingGuideCardAgendaItem?.id;
      if (dismissRaisedHand && isHandRaised && currentAgendaModelItemId != null) {
    ...
    final isTest =
        (routerDelegate.currentBeamLocation?.state as BeamState).queryParameters['test'] != null;
    if (isTest) {
      if (!(_room?.localParticipant?.audioTrackEnabled ?? false)) {
        await AudioVideoErrorDialog.showOnError(
            navigatorState.context, () => toggleAudioEnabled(setEnabled: true));
      }
      if (!(_room?.localParticipant?.videoTrackEnabled ?? false)) {
        await AudioVideoErrorDialog.showOnError(
            navigatorState.context, () => toggleVideoEnabled(setEnabled: true));
      }
  ...
  void _updateLiveMeetingParticipants({
    ...
    participantInitializationTimers.removeWhere((id, _) => !participantIds.contains(id));
    ...
    liveMeetingProvider.setMeetingProviderParticipants(
      participants
          .map((p) => VideoParticipant(p, liveMeetingProvider.discussionProvider.discussion,
              liveMeetingProvider.discussionPath))
          .toList(),
      notify: notify,
    );
  }
  ...
  void onParticipantDisconnected() {
    ...
    if (liveMeetingProvider.isInBreakout) {
      Future.delayed(Duration(milliseconds: (5.0 * random.nextDouble() * 1000).round()), () {
        if (!_isDisposed) {
  ...
  /// it will throw.
  static ConferenceRoom watch(BuildContext context) => Provider.of<ConferenceRoom>(context);
