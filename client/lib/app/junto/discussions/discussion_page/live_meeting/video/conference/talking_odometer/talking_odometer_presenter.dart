...
import 'package:junto/app/junto/discussions/discussion_page/live_meeting/video/conference/talking_odometer/talking_odometer_contract.dart';
import 'package:junto/app/junto/discussions/discussion_page/live_meeting/video/conference/talking_odometer/talking_odometer_model.dart';
...
class TalkingOdometerPresenter {
  ...
  void incrementDuration(String userId) =>
      _model.userSpeakingDurations[userId] = (_model.userSpeakingDurations[userId] ?? 0) + 1;
  ...
  })  : _helper = talkingOdometerPresenterHelper ?? TalkingOdometerPresenterHelper(),
        _conferenceRoom = conferenceRoom ?? LiveMeetingProvider.read(context).conferenceRoom!,
        discussionPermissions = context.read<DiscussionPermissionsProvider>();
  ...
  void _checkDominantSpeakerUpdates() {
    ...
    final isLocalParticipantSpeaking =
        _model.currentDominantSpeakerSid == _conferenceRoom.room?.localParticipant?.userId;
    TalkingState newTalkingState = _model.talkingState;

    // If the current speaker is no one, keep the state of the last speaker
    if (dominantSpeakerSid != null) {
      newTalkingState = isLocalParticipantSpeaking ? TalkingState.talking : TalkingState.idle;
    }
    ...
    } else if (newTalkingState == TalkingState.talking && isLocalParticipantSpeaking) {
  ...
  void _setDialStateAndTooltip(TalkingState newTalkingState) {
    ...
    final threshold = _helper.getDurationThreshold(
        newTalkingState, _conferenceRoom, _conferenceRoom.participants.length);

    bool hasntSpokenInAWhile =
        newTalkingState == TalkingState.idle && threshold < lastSpokenDuration;

    bool speakingTooMuch =
        newTalkingState == TalkingState.talking && threshold < _model.talkingStateDuration;
    ...
    if ((hasntSpokenInAWhile || speakingTooMuch) && !_model.tooltipAlreadyShown) {
      _model.tooltipAlreadyShown = true;
      showTooltip();

      // Only show tooltip for certain amount of time. Once this duration is reached - hide tooltip.
      _model.tooltipShownTimer = Timer(TalkingOdometerPresenterHelper._kTooltipShowDuration, () {
        hideTooltip();
      });
    }
  }
      ...
      case DialState.value:
        ...
        final userSpeakingProportion = (userTotalTalkingTime.inMilliseconds +
            ...
            (totalTalkingTimeInMeeting.inMilliseconds +
                (_conferenceRoom.participants.length * startingSpeakingDuration.inMilliseconds));
        ...
        final normalizedProportion = Normal.cdf(userSpeakingProportion,
            mean: expectedProportion, variance: expectedProportion * .5);
  ...
  String getMessage() {
    ...
    if (_model.talkingStateDuration >= durationThreshold - Duration(seconds: 1) ||
        DateTime.now().difference(_model.lastSpokenTime) >=
            durationThreshold - Duration(seconds: 1)) {
      return message;
    } else {
      final speakingTime = userTotalTalkingTime.getFormattedTime(showHours: false);
...
class TalkingOdometerPresenterHelper {
  ...
  static Duration _idleThreshold(int participants) => Duration(minutes: 2 * participants);
  ...
  Duration getDurationThreshold(
      TalkingState talkingState, ConferenceRoom? conferenceRoom, int participants) {
    if (conferenceRoom?.juntoProvider.isDeliberations ?? false) {
      switch (talkingState) {
        case TalkingState.talking:
          return _deliberationsTalkingThreshold;
        case TalkingState.idle:
          return _deliberationsIdleThreshold;
      }
    }
