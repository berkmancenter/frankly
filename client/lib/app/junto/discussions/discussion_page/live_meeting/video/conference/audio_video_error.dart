...
class AudioVideoErrorDialog extends StatelessWidget {
  ...
  const AudioVideoErrorDialog({Key? key, required this.error}) : super(key: key);

  static Future<T?> showOnError<T>(BuildContext context, Future<T> Function() action) async {
    try {
      return await action();
    } catch (e, s) {
      loggingService.log('Error in audio/video', logType: LogType.error, error: e, stackTrace: s);

      await showJuntoDialog(
          context: context, builder: (_) => AudioVideoErrorDialog(error: e.toString()));
      return null;
    }
  }
  ...
  static Future<void> show<T>(BuildContext context, String error) async {
      await showJuntoDialog(
          context: context, builder: (_) => AudioVideoErrorDialog(error: error));
  }
...
class AudioVideoErrorDisplay extends StatelessWidget {
  ...
  const AudioVideoErrorDisplay({Key? key, required this.error, this.textColor = AppColor.white})
      : super(key: key);

  String _buildErrorText() {
    String errorText = error;
    if (errorText.contains('NotReadableError')) {
      errorText = 'Unable to start microphone or camera. Ensure that no other app or tab is using '
    ...
    } else if (['OverconstrainedError', 'TypeError'].any((error) => errorText.contains(error))) {
      errorText = 'Error getting camera and microphone media. Please refresh.';
    } else if (errorText.contains('NotAllowedError')) {
      errorText = 'You must give permission to access your microphone and camera.'
    ...
    } else if (errorText.toLowerCase().contains('Invalid constraints'.toLowerCase())) {
      errorText = 'Error connecting to microphone or camera. Please ensure your browser is updated'
          ' and try again.';
    } else if (errorText.contains('TimeoutException')) {
      errorText = 'Oops there was a network error, please try that again.';
    }

    if (errorText.contains('TwilioError: Participant disconnected because of duplicate identity')) {
      errorText = 'Looks like you have successfully rejoined from a different device or browser.';
    }
