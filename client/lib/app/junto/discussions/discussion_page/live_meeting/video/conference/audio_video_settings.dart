...
import 'package:junto/common_widgets/troubleshoot_av.dart';
...
class AudioVideoSettingsDialog extends HookWidget {
  ...
  Future<void> show() {
    return showJuntoDialog(builder: (context) => this);
  }
  ...
  List<Widget> _buildVideoDevicesDropdown({
    ...
    final currentDeviceFound = devices.any((d) => d.deviceId == currentDeviceId);
      ...
      if (devices.isEmpty)
        ...
        Container(
          ...
          child: DropdownButton<String>(
            underline: SizedBox(),
            onChanged: (deviceId) => onChanged(devices.firstWhere((d) => d.deviceId == deviceId)),
  ...
  Future<List<VideoDeviceInfo>> _getVideoDevices() async {
    try {
      return await conferenceRoom.room?.engine.getVideoDeviceManager().enumerateVideoDevices() ??
          [];
    } catch (e) {
      loggingService.log('Error in AudioVideoSettingsDialog.getVideoDevices: $e');
      return [];
    }
  }
  ...
  Future<List<AudioDeviceInfo>> _getAudioDevices() async {
    try {
      ...
      loggingService.log('Error in AudioVideoSettingsDialog.getAudioDevices: $e');
      return [];
    }
  }
                          ...
                          children: [
                            ...
                            ..._buildAudioDevicesDropdown(
                              onChanged: (device) {
                                ...
                                if (id != null &&
                                    sharedPreferencesService.getDefaultMicrophoneId() != id) {
                                  sharedPreferencesService.setDefaultMicrophoneId(id);
                                  if (conferenceRoom.audioEnabled) {
                                    conferenceRoom.toggleAudioEnabled(setEnabled: true);
                                  }
                                }
                              },
                              currentDeviceId: sharedPreferencesService.getDefaultMicrophoneId(),
                            ...
                            ..._buildVideoDevicesDropdown(
                              onChanged: (device) {
                                final id = device.deviceId;
                                if (id != null &&
                                    sharedPreferencesService.getDefaultCameraId() != id) {
                                  sharedPreferencesService.setDefaultCameraId(id);
                                  if (conferenceRoom.videoEnabled) {
                                    conferenceRoom.toggleVideoEnabled(setEnabled: true);
                                  }
                                }
                              },
                              currentDeviceId: sharedPreferencesService.getDefaultCameraId(),
