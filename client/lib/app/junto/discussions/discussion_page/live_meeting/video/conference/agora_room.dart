...
import '../../live_meeting_provider.dart';
...
class AgoraRoom with ChangeNotifier {
  ...
  BehaviorSubject<AgoraParticipant?> get dominantSpeakerStream => _dominantSpeakerStream;

  void _updateDominantSpeaker({int? localUid, required List<AudioVolumeInfo> speakers}) {
    ...
    final localSpeaker = speakers.where((s) => localUid != null && s.uid == localUid).firstOrNull;
    ...
    if (dominantRemoteSpeaker != null && (dominantRemoteSpeaker.volume ?? 0) > volumeCutoff) {
  ...
  Future<void> connect({bool enableAudio = false, bool enableVideo = false}) async {
    ...
    _rtcEngineEventHandler = RtcEngineEventHandler(
      onError: (ErrorCodeType err, String msg) {
        ...
        if (err == ErrorCodeType.errJoinChannelRejected) {
          conferenceRoom.setConnectError('Could not join room. Please refresh and try again');
        }
      },
      ...
      onUserJoined: (RtcConnection connection, int rUid, int elapsed) async {
        ...
        String? userId;
        try {
          final user = await cloudFunctionsService
      ...
      onUserMuteVideo: (RtcConnection connection, int rUid, bool muted) {
        print('[onUserMuteVideo] connection: ${connection.toJson()} muted: $muted');

        _videoMutedState[rUid] = muted;
        _remoteParticipants.where((p) => p.agoraUid == rUid).firstOrNull?.videoTrackEnabled =
            !muted;
        notifyListeners();
      },
      onUserMuteAudio: (RtcConnection connection, int rUid, bool muted) {
        print('[onUserMuteAudio] connection: ${connection.toJson()} muted: $muted');
        _audioMutedState[rUid] = muted;
        _remoteParticipants.where((p) => p.agoraUid == rUid).firstOrNull?.audioTrackEnabled =
            !muted;

        notifyListeners();
      },
      onUserEnableVideo: (RtcConnection connection, int rUid, bool enabled) {
        print('[onUserEnableVideo] connection: ${connection.toJson()} enabled: $enabled');
      },
      onVideoSubscribeStateChanged: (String channelId, int uid, StreamSubscribeState oldState,
          StreamSubscribeState newState, int elapsedTime) {
        print(
      ...
      onUserEnableLocalVideo: (RtcConnection connection, int uid, bool enabled) {
        print(
            '[onUserEnableLocalVideo] connection: ${connection.toJson()} uid: $uid enabled: $enabled');
        notifyListeners();
      },
      onLocalAudioStateChanged:
          (RtcConnection connection, LocalAudioStreamState state, LocalAudioStreamReason reason) {
        print(
            '[onLocalAudioStateChanged] connection: ${connection.toJson()} state: $state reason: $reason');
      },
      onNetworkQuality: (RtcConnection connection, int uid, QualityType txQuality, rxQuality) {
        if (uid == 0) {
          _localParticipant?.networkQualityLevel = txQuality;
        } else {
          _remoteParticipants.where((p) => p.agoraUid == uid).firstOrNull?.networkQualityLevel =
              txQuality;
        }
        notifyListeners();
      },
      onAudioVolumeIndication: (RtcConnection connection, List<AudioVolumeInfo> speakers,
        ...
        for (final speaker in speakers) {
          if (speaker.uid == connection.localUid) {
            ...
            final participant =
                _remoteParticipants.where((s) => s.agoraUid == speaker.uid).firstOrNull;
            participant?.volume = speaker.volume;
          }
        }

        _updateDominantSpeaker(localUid: connection.localUid, speakers: speakers);
      },
    );
...
class AgoraParticipant with ChangeNotifier {
  ...
  Future<void> enableAudio({required bool setEnabled, String? deviceId}) async {
    if (setEnabled) {
      ...
      await _rtcEngine.updateChannelMediaOptions(ChannelMediaOptions(
