...
class BradyBunchLayout {
  ...
  bool get isWidthLimited => v.totalWidth * p.rows < v.totalHeight * (p.columns * (p.aspectRatio));
  ...
  double get deadSpace {
    ...
    return totalSpace -
        bradySpace -
        (switchAspectOnLastRow ? imageArea * (getAdjustedAspectRatio - 1) * bradysOnLastRow : 0);
  }
  ...
  })  : assert(layoutValues.numBradys <= layoutParameters.rows * layoutParameters.columns,
            'Too many Bradys'),
        assert(layoutValues.numBradys > layoutParameters.columns * (layoutParameters.rows - 1),
            'Not enough Bradys');
  ...
  factory BradyBunchLayout.calculateOptimalLayout({
    ...
    final vals = BradyBunchLayoutValues(totalWidth: width, totalHeight: height, numBradys: 1);
    ...
    do {
      ...
      layoutToTest = layoutToTest.exploreRowAndColumnVariations(participantCount);
    } while (
        layoutToTest.layoutValues.numBradys < participantCount || previousLayout != layoutToTest);
    ...
    if (!layoutToTest.isWidthLimited) {
      ...
      final normalizedColumns = (layoutToTest.layoutValues.numBradys / layoutToTest.rows).ceil();
      if (normalizedColumns != layoutToTest.columns) {
        final normalizedLayout =
            layoutToTest.setParams(columns: normalizedColumns).expandHorizontally();
    ...
    if (layoutToTest.columns > 1) {
      if (layoutToTest.rows >= layoutToTest.columns) {
        ...
        final adjustedRows =
            (layoutToTest.layoutValues.numBradys / (layoutToTest.columns - 1)).ceil();
        ...
        final adjustedColumns =
            (layoutToTest.layoutValues.numBradys / (layoutToTest.rows + 1)).ceil();
  ...
  BradyBunchLayout exploreRowAndColumnVariations(int participantCount) {
    ...
    if (vals.numBradys == participantCount) {
      ...
      if (participantCount > rows * (columns + 1)) {
        manipulateRowAndColumnLayout = BradyBunchLayout(
          ...
          layoutValues: vals.withBradyCount(atMost(vals.numBradys + rows + columns + 1)),
        );
        manipulateRowAndColumnDeadspace = manipulateRowAndColumnLayout.deadSpace;
      }
    }
    ...
    final minDeadspace = (validDeadspaces..sort((a, b) => a.compareTo(b))).first;
  ...
  BradyBunchLayout expandHorizontally() {
    final maxUnboundedAspectRatio = layoutValues.totalWidth / (imageSize.height * columns);
