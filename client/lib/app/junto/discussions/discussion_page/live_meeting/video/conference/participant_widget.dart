...
class ParticipantWidget extends StatefulWidget {
  ...
  final JuntoGlobalKey globalKey;
...
class _ParticipantWidgetState extends State<ParticipantWidget> {
  ...
  bool get isDominant => conferenceRoom.dominantSpeakerSid == widget.participant.userId;
  ...
  bool get _isNewlyConnected {
    final timer = conferenceRoom.participantInitializationTimers[widget.participant.userId];
    return timer?.isActive ?? false;
  }

  bool get audioEnabled =>
      isRemote ? _isRemoteAudioEnabled(widget.participant.userId) : conferenceRoom.audioEnabled;

  bool get isStarted => widget.participant.videoTrackEnabled;

  bool _isRemoteAudioEnabled(String participantId) {
    final hasMuteOverride = DiscussionProvider.watch(context)
  ...
  Widget _buildOverlay() {
    final bool isHandRaised =
        Provider.of<MeetingGuideCardStore>(context).getHandIsRaised(widget.participant.identity);
    final conferenceRoom = ConferenceRoom.watch(context);
    final handRaisedIndex = conferenceRoom.handRaisedParticipants.indexOf(widget.participant);
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: Align(
              alignment: Alignment.bottomLeft,
              child: isMobile ? _buildParticipantDetailsMobile() : _buildParticipantDetails(),
            ),
          ),
          if (isDominant)
  ...
  Widget _buildEndRowIcon(AppAsset appAsset) {
    ...
    return Padding(
      ...
      child: JuntoImage(null, asset: appAsset, width: size, height: size),
    );
  }
  ...
  Widget _buildParticipantDetails() {
    final showPin = context.watch<DiscussionPermissionsProvider>().canPinItemInParticipantWidget;
          ...
          children: [
            Flexible(
              child: UserInfoBuilder(
                ...
                builder: (_, isLoading, snapshot) => JuntoText(
                  isLoading ? 'Loading...' : snapshot.data?.displayName ?? 'Participant',
            ...
            _ParticipantOptionsMenu(
              ...
              showMute: showMute,
              showKick: showKick,
              isVisible: _showName || !responsiveLayoutService.isMobile(context),
            ),
          ],
        ),
      ),
  ...
  Widget _buildVideoDisabled({bool switchedOff = false}) {
    final isConnecting = _isNewlyConnected || (_startedTimer?.isActive ?? false);
    final isMobile = responsiveLayoutService.isMobile(context);
...
class _ParticipantOptionsMenu extends StatefulWidget {
  ...
  @override
  State<_ParticipantOptionsMenu> createState() => _ParticipantOptionsMenuState();
}
...
class _ParticipantOptionsMenuState extends State<_ParticipantOptionsMenu> {
  ...
  List<PopupMenuEntry<Function()>> _getMenuItems({required BuildContext context}) {
    final liveMeetingProvider = LiveMeetingProvider.read(context);
    final userId = widget.userId;
    final isPinned = _isPinnedLocal ??
        liveMeetingProvider.liveMeeting?.pinnedUserIds.any((id) => id == widget.userId) ??
        false;
    ...
    return [
      ...
      if (widget.showMute)
        PopupMenuItem<Function()>(
          value: () => alertOnError(context, () => liveMeetingProvider.mute(userId: userId)),
          child: JuntoText(
            'Mute',
            style: AppTextStyle.bodyMedium.copyWith(color: AppColor.darkBlue),
          ),
      ...
      if (widget.showKick)
        PopupMenuItem<Function()>(
          value: () => alertOnError(context, () => liveMeetingProvider.confirmProposeKick(userId)),
          child: JuntoText(
            'Propose to remove user',
            style: AppTextStyle.bodyMedium.copyWith(color: AppColor.redLightMode),
          ),
        ),
      PopupMenuItem<Function()>(
        value: () => alertOnError(
          ...
          () => Dialogs.showAppDrawer(
            ...
            AppDrawerSide.right,
            ProfileTab(
              juntoId: liveMeetingProvider.juntoProvider.juntoId,
              showTitle: true,
  ...
  Widget build(BuildContext context) {
    final isPinned = _isPinnedLocal ??
        ...
        false;
    return JuntoInkWell(
      onTap: widget.isVisible ? () => _showMoreMenu(_getMenuItems(context: context)) : null,
      onHover: widget.isVisible ? (isHovered) => setState(() => _isHovered = isHovered) : null,
      child: Container(
        key: _menuKey,
