...
class MeetingGuideCardItemWordCloud extends StatefulWidget {
  ...
  @override
  _MeetingGuideCardItemWordCloudState createState() => _MeetingGuideCardItemWordCloudState();
}

class _MeetingGuideCardItemWordCloudState extends State<MeetingGuideCardItemWordCloud>
    implements MeetingGuideCardItemWordCloudView {
  final FocusNode _wordCloudResponseFocusNode = FocusNode();
  final TextEditingController _wordCloudResponseController = TextEditingController(text: '');
  ...
  Future<void> _submitWordCloudResponse(String response) async {
    await alertOnError(context, () => _presenter.addWordCloudResponse(response));
  }
  ...
  Widget _buildWordCloudContent() {
    ...
    final currentDiscussion = _presenter.getDiscussion();
    ...
    return MemoizedBuilder<Stream<List<ParticipantAgendaItemDetails>>?>(
      getter: () {
        return inLiveMeeting ? participantAgendaItemDetailsStream : Stream.value([]);
      },
      keys: [currentDiscussion, currentAgendaItem?.id ?? '', inLiveMeeting],
        ...
        return JuntoStreamBuilder<List<ParticipantAgendaItemDetails>>(
          ...
          builder: (_, details) {
            ...
            final allResponses = groupBy<String, String>(
                    (details ?? []).expand((d) => d.wordCloudResponses), (word) => word)
                .map<String, int>((k, v) => MapEntry(k, v.length))
                .entries
                .sorted((a, b) =>
                    a.value == b.value ? a.key.compareTo(b.key) : -a.value.compareTo(b.value));
  ...
  Widget _buildWordCloudList(List<MapEntry<String, int>> allResponses, int maxValue) {
    return Center(
      child: LayoutBuilder(
        builder: (_, constraints) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final lineWidth = maxLineWidth * (response.value.toDouble() / maxValue);
                    ...
                    Tooltip(
                      ...
                      child: ClipRRect(
                        ...
                        child: Container(
                          height: 8,
                          width: lineWidth,
                          decoration: BoxDecoration(color: AppColor.brightGreen),
                        ),
                      ),
                    ),
                    SizedBox(width: 10),
        ...
        builder: (context, words) {
          return Scatter(
            ...
            children: words.map((e) {
              final prominence = maxValue > 1 ? (e.value.floorToDouble() - 1) / (maxValue - 1) : .5;
              final size =
                  24 * (1.5 + prominence) / math.sqrt(math.sqrt(math.max(words.length, 6)));
          ...
          currentResponses.length,
            ...
            return Chip(
              ...
              onDeleted: () {
                  alertOnError(
                    context,
                        () => _presenter.removeWordCloudResponse(currentResponses[index]),
                  );
              },
