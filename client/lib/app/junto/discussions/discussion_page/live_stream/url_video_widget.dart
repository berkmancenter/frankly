...
class _UrlVideoWidgetState extends State<UrlVideoWidget> {
  ...
  String get encodedUrl {
    ...
    if (playbackUrl.startsWith('http://') && playbackUrl.contains('cloudinary')) {
    ...
    useEffect(() {
      final subscription = html.window.onMessage.listen((event) {
        ...
        if (messageObj['source'] == 'videojs') {
          ...
          print(messageObj);
          print('currentTime type: ${messageObj['currentTime']?.runtimeType}');
          print('videoDuration type: ${messageObj['videoDuration']?.runtimeType}');
          ...
          if (messageType == 'video-update') {
            loggingService.log('message update event received: ${event.data}');
...
class _UrlVideoInternal extends StatefulWidget {
  ...
  const _UrlVideoInternal({required Key key, required this.url}) : super(key: key);
...
class _UrlVideoInternalState extends State<_UrlVideoInternal> {
  ...
  void initState() {
    ...
    loggingService.log('reinitializing state of url video widget: ${widget.url}');
