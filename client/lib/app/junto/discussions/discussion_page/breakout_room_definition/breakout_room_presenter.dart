...
class BreakoutRoomPresenter extends ChangeNotifier {
  static final BreakoutRoomDefinition defaultBreakoutRoomDefinition = BreakoutRoomDefinition(
  ...
  BreakoutRoomDefinition get breakoutRoomDefinitionDetails => _breakoutRoomDefinition;

  void initialize() {
    _breakoutRoomDefinition = discussionProvider.discussion.breakoutRoomDefinition ??
        discussionProvider.defaultBreakoutRoomDefinition;
    addInitialCategories();
  }

  Future<void> updateAssignmentMethod({required BreakoutAssignmentMethod assignmentMethod}) async {
    _breakoutRoomDefinition = _breakoutRoomDefinition.copyWith(assignmentMethod: assignmentMethod);
    await _updateDiscussionDetails();
  }

  Future<void> updateParticipants({required int participantsNumber}) async {
    _breakoutRoomDefinition =
        _breakoutRoomDefinition.copyWith(targetParticipants: participantsNumber);
    await _updateDiscussionDetails();
  }

  void addQuestion() {
    final questions = _breakoutRoomDefinition.breakoutQuestions;
    if (questions.isNotEmpty && !isQuestionAndAnswersCompleted(questions.last)) {
      showRegularToast('Please complete previous question and both answers', ToastType.failed);
    } else {
  ...
  void updateQuestionData({
    ...
    final breakoutQuestionIndex = _breakoutRoomDefinition.breakoutQuestions.indexWhere(
    ...
    _breakoutRoomDefinition.breakoutQuestions[breakoutQuestionIndex] =
        _breakoutRoomDefinition.breakoutQuestions[breakoutQuestionIndex].copyWith(title: question);
    notifyListeners();
  }
  ...
  void updateQuestionAnswerData({
    ...
    final questionIndex =
        _breakoutRoomDefinition.breakoutQuestions.indexWhere((element) => element.id == questionId);
    ...
    final answerIndex = question.answers.indexWhere(
      (answer) => answer.options.any((option) => option.id == breakoutAnswerOption.id),
    );
    final answer = question.answers[answerIndex];
    final optionIndex =
        answer.options.indexWhere((element) => element.id == breakoutAnswerOption.id);

    _breakoutRoomDefinition
        .breakoutQuestions[questionIndex].answers[answerIndex].options[optionIndex] = option;
  ...
  Future<bool> updateBreakoutRoomQuestion() async {
    ...
    if (!areAllQuestionsCompleted) {
      showRegularToast('Please complete all your questions and answers!', ToastType.failed);
      return false;
    } else {
      await _updateDiscussionDetails();
  ...
  Future<void> deleteBreakoutRoomQuestion(String questionId) async {
    _breakoutRoomDefinition.breakoutQuestions.removeWhere((element) => element.id == questionId);
    await _updateDiscussionDetails();
    showRegularToast('Question deleted!', ToastType.success);
  }

  Future<void> _updateDiscussionDetails() async {
    _breakoutRoomDefinition.breakoutQuestions.removeWhere((q) => isNullOrEmpty(q.title));
  ...
  void reorderQuestions({required int draggedIndex, required int newPositionIndex}) {
    final removed = breakoutRoomDefinitionDetails.breakoutQuestions.removeAt(draggedIndex);
    breakoutRoomDefinitionDetails.breakoutQuestions.insert(newPositionIndex, removed);
    notifyListeners();
  }
  ...
  Future<void> saveReorder() async {
    ...
    if (areAllQuestionsCompleted) {
      ...
      showRegularToast('Reordering not saved until all questions are completed.', ToastType.failed);
    }
  }

  void addCategory() {
    _breakoutRoomDefinition.categories.add(BreakoutCategory(id: uuid.v4(), category: ''));
    notifyListeners();
  }

  void updateCategoryData({required String category, required int position}) {
    _breakoutRoomDefinition.categories[position] =
        _breakoutRoomDefinition.categories[position].copyWith(category: category);
  ...
  Future<void> updateBreakoutRoomCategory() async {
    _breakoutRoomDefinition.categories.removeWhere((q) => isNullOrEmpty(q.category));
    if (_breakoutRoomDefinition.categories.length < 2) {
      return showRegularToast('You must add atleast two (2) categories.', ToastType.failed);
    }
  ...
  String getLabelText(
    ...
    final answerOptionPosition = breakoutAnswerOptions.indexWhere((element) => element.id == id);
  ...
  void removeAnswerOption(BreakoutAnswer breakoutAnswer, String answerOptionId) {
    breakoutAnswer.options.removeWhere((element) => element.id == answerOptionId);
    notifyListeners();
  }
}
