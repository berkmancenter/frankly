...
class PrePostCardWidgetPresenter {
  ...
  })  : _helper = prePostCardWidgetPresenterHelper ?? PrePostCardWidgetPresenterHelper(),
        _cloudFunctionsService = testCloudFunctionsService ?? cloudFunctionsService,
        _userService = userService ?? Provider.of<UserService>(context, listen: false);

  void init(PrePostCardWidgetType prePostCardWidgetType, PrePostCard? prePostCard) {
    _model.prePostCard = prePostCard ?? PrePostCard.newCard(_model.prePostCardType);
  ...
  String? validateButtonText(String? text, int urlIndex) {
    ...
    final urlIsNotEmpty = urlIndex < prePostUrls.length && prePostUrls[urlIndex].surveyUrl != null;
  ...
  List<PrePostCardAttributeType> getAvailableAttributeTypes(
    ...
    for (var selectedAttribute in selectedAttributes) {
      prePostCardAttributeTypes.removeWhere((element) => element == selectedAttribute.type);
    }
  ...
  bool isEditIconShown() {
    return _model.isEditable && _model.prePostCardWidgetType == PrePostCardWidgetType.overview;
  }
  ...
  void addNewURLParamRow(int urlIndex) {
    ...
    if (urlIndex < prePostUrls.length) {
      ...
      final List<PrePostCardAttribute> attributes = List.of(prePostUrls[urlIndex].attributes);

      prePostUrls[urlIndex] = prePostUrls[urlIndex].copyWith(attributes: [
        ...attributes,
  ...
  void updateCard(int urlIndex) {
    ...
    if (urlIndex < prePostUrls.length) {
      ...
      prePostUrls[urlIndex] = prePostUrls[urlIndex].copyWith(attributes: attributes);
      _model.prePostCard = _model.prePostCard.copyWith(prePostUrls: prePostUrls);
  ...
  PrePostCard getPrePostCardDetailsToSave() {
    var prePostUrls = _model.prePostCard.prePostUrls;
    prePostUrls
        .removeWhere((link) => isNullOrEmpty(link.buttonText) || isNullOrEmpty(link.surveyUrl));

    prePostUrls = prePostUrls
        .map((link) => link.copyWith(
            attributes: link.attributes
                .map((attribute) => attribute.copyWith(
  ...
  void updateAttributeTypeSelection(
      PrePostCardAttributeType? selectedType, int urlIndex, int attributeIndex) {
    if (selectedType != null) {
      var prePostUrls = _model.prePostCard.prePostUrls;
      if (urlIndex < prePostUrls.length) {
        final PrePostCardAttribute prePostCardAttribute =
            prePostUrls[urlIndex].attributes[attributeIndex].copyWith(type: selectedType);
  ...
  void updateEnteredQueryName(
      int urlIndex, int attributeIndex, PrePostCardAttribute attribute, text) {
    var prePostUrls = _model.prePostCard.prePostUrls;

    if (urlIndex < prePostUrls.length) {
      prePostUrls[urlIndex].attributes[attributeIndex] = attribute.copyWith(queryParam: text);
      _model.prePostCard.copyWith(prePostUrls: prePostUrls);
      _view.updateView();
    }
  }
  ...
  void updateEnteredUrl(String text, int urlIndex) {
    ...
    if (urlIndex < prePostUrls.length) {
      prePostUrls[urlIndex] = prePostUrls[urlIndex].copyWith(surveyUrl: manipulatedText);
      _model.prePostCard = _model.prePostCard.copyWith(prePostUrls: prePostUrls);
      _view.updateView();
    }
  }
  ...
  void updateEnteredButtonText(int urlIndex, String text) {
    ...
    if (urlIndex < prePostUrls.length) {
      prePostUrls[urlIndex] = prePostUrls[urlIndex].copyWith(buttonText: manipulatedText);
      _model.prePostCard = _model.prePostCard.copyWith(prePostUrls: prePostUrls);
    }
  }
  ...
  bool hasBeenEdited(Discussion? discussion) {
    final prePostCard = _model.prePostCardType == PrePostCardType.preEvent
        ? discussion?.preEventCardData
        : discussion?.postEventCardData;
  ...
  void deleteQueryParamRow(int urlIndex, int attributeIndex) {
    ...
    if (urlIndex < prePostUrls.length) {
      List<PrePostCardAttribute> attributes = List.of(prePostUrls[urlIndex].attributes);
  ...
  Future<String?> getEmail(
    ...
    try {
      getUserAdminDetailsResponse =
          await cloudFunctionsService.getUserAdminDetails(GetUserAdminDetailsRequest(
        userIds: [userId],
      ));
    } catch (e, s) {
      loggingService.log('PrePostCardWidgetPresenterHelper.getEmail', error: e, stackTrace: s);
    }
