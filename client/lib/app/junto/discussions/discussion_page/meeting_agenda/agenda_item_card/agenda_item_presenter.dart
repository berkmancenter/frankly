...
class AgendaItemPresenter {
  ...
  final DiscussionPermissionsProvider? _discussionPermissionsProvider;
  ...
  AgendaItemPresenter(
    ...
    TopicPageProvider? topicPageProvider,
        ...
        _meetingGuideCardModel = meetingGuideCardStore ?? MeetingGuideCardStore.read(context),
        _discussionPermissionsProvider =
            discussionPermissionsProvider ?? DiscussionPermissionsProvider.read(context),
        _liveMeetingProvider = liveMeetingProvider ?? LiveMeetingProvider.readOrNull(context),
  ...
  bool isCollapsed() {
    return _agendaProvider.collapsedAgendaItemIds.contains(_model.agendaItem.id);
  }
  ...
  void changeAgendaType(AgendaItemType type) {
    switch (type) {
      ...
      case AgendaItemType.userSuggestions:
        _model.agendaItemUserSuggestionsData = AgendaItemUserSuggestionsData.newItem();
        break;
    }
  ...
  void duplicateCard() {
    ...
    _view.showMessage('Agenda item was duplicated!', toastType: ToastType.success);
  }
  ...
  Future<void> saveContent() async {
    ...
    if (wasEdited) {
      ...
      loggingService.log('AgendaItemPresenter.saveContent: Update: ${updatedItem.toJson()}');
  ...
  bool isCardUnsaved() {
    return _agendaProvider.unsavedItems.any((element) => element.id == _model.agendaItem.id);
  }
  ...
  bool doesAllowEdit() {
    final _params = _agendaProvider.params;
    if (_params.isNotOnDiscussionPage) {
      final Topic? localTopic = _params.topic;
      return localTopic != null ? _communityPermissionsProvider.canEditTopic(localTopic) : false;
    } else {
      final canEditDiscussion = _discussionPermissionsProvider?.canEditDiscussion ?? false;
      final isInBreakout = _liveMeetingProvider?.isInBreakout ?? false;

      return canEditDiscussion && !isInBreakout;
    }
  }
  ...
  bool canReorder(bool allowEdit, bool isEditMode, bool isCompleted, bool isCardActive) {
    ...
    switch (agendaItemType) {
      ...
      case AgendaItemType.video:
        return agendaItemTitle != agendaItemModel.agendaItemVideoData.title ||
            (agendaItem.videoUrl ?? '') != agendaItemModel.agendaItemVideoData.url ||
      ...
      case AgendaItemType.image:
        return agendaItemTitle != agendaItemModel.agendaItemImageData.title ||
            (agendaItem.imageUrl ?? '') != agendaItemModel.agendaItemImageData.url;
      case AgendaItemType.poll:
        return agendaItemContent != agendaItemModel.agendaItemPollData.question ||
            !ListEquality().equals(
              agendaItemModel.agendaItemPollData.answers,
              agendaItem.pollAnswers ?? [],
            );
      case AgendaItemType.wordCloud:
        return agendaItemContent != agendaItemModel.agendaItemWordCloudData.prompt;
      case AgendaItemType.userSuggestions:
        return agendaItemTitle != agendaItemModel.agendaItemUserSuggestionsData.headline;
    }
  }

  void initialiseFields(AgendaItemModel agendaItemModel) {
    agendaItemModel.timeInSeconds =
        agendaItemModel.agendaItem.timeInSeconds ?? AgendaItem.kDefaultTimeInSeconds;
    ...
    agendaItemModel.agendaItemUserSuggestionsData = AgendaItemUserSuggestionsData(
      agendaItemModel.agendaItem.title ?? '',
    );
  }
