...
class _ShareSectionState extends State<ShareSection> {
  ...
  String _withCacheBuster(String url) {
    ...
    final newUri = uri.replace(queryParameters: {'b': rand.toString(), ...uri.queryParameters});
    return newUri.toString();
  }

  void _tapFacebook() {
    final query = Uri(queryParameters: {'u': _withCacheBuster(widget.url)}).query;
  ...
  void _tapTwitter() {
    final query =
        Uri(queryParameters: {'url': _withCacheBuster(widget.url), 'text': widget.body}).query;
  ...
  void _tapLinkedIn() {
    final query = Uri(queryParameters: {'url': _withCacheBuster(widget.url)}).query;
  ...
  void _tapEmail() {
    final link = Uri(scheme: 'mailto', queryParameters: {
  ...
  Widget _buildFAIcon(
    ...
    final size = widget.size ?? responsiveLayoutService.getDynamicSize(context, 40.0);
    final iconSize = widget.iconSize ?? responsiveLayoutService.getDynamicSize(context, 20.0);
    return Padding(
