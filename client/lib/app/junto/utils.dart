...
Future<void> showAlert(BuildContext context, String alert) => showJuntoDialog<void>(
      context: context,
  ...
  FToast().init(context).showToast(
      child: Container(
        ...
        child: Row(
          ...
          children: [
            if (iconPath != null) ...[
              SvgPicture.asset(iconPath.path, color: textColor, width: 20, height: 20),
              SizedBox(width: 10),
            ],
            Flexible(
              child: Text(
...
Future<T?> alertOnError<T>(
  ...
  try {
    ...
    final _sanitizedError = sanitizeError(e.toString());

    await showAlert(context, errorMessage ?? _sanitizedError);
    return null;
  }
}
...
Future<void> popOrShowError(BuildContext context, Future<void> Function() action) =>
    alertOnError(context, () async {
      await action();
      Navigator.of(context).pop();
    });

Future<T?> guardSignedIn<T>(Future<T> Function() action,
    {bool isPurchasingSubscription = false}) async {
...
Future<T?> guardJuntoMember<T>(BuildContext context, Junto junto, Future<T> Function() action) {
  final juntoId = junto.id;

  return guardSignedIn<T?>(() async {
    await juntoUserDataService.memberships.first;
    if (!juntoUserDataService.getMembership(juntoId).isMember) {
      final joinJunto = await ConfirmDialog(
        title: 'Join ${junto.name}?',
        mainText: 'You must be a member of this space to participate. Would you like to join?',
        confirmText: 'Yes, Join!',
      ).show();

      if (!joinJunto) return null;
    }
...
Future<T?> swallowErrors<T>(FutureOr<T> Function() action, {String? errorMessage}) async {
  try {
    return await action();
  } catch (e, stackTrace) {
...
Future<void> launch(
  ...
  final webUrl = !isWeb || url.startsWith(RegExp('https?://')) ? url : 'https://$url';
...
class HeaderHorizontalPadding extends StatelessWidget {
  ...
  EdgeInsets _padding(BuildContext context) =>
      EdgeInsets.symmetric(horizontal: responsiveLayoutService.isMobile(context) ? 18 : 30);
...
String durationString(Duration duration, {bool readAsHuman = false}) {
  ...
  if (duration.inHours > 0) {
    ...
    return readAsHuman ? '$twoDigitMinutes min' : '$twoDigitMinutes:$twoDigitSeconds';
  }
}

List<T> ifNotNull<T>(T item) => [if (item != null) item];

class JuntoLoadingIndicator extends StatelessWidget {
  final double strokeWidth;
  final Color? color;

  const JuntoLoadingIndicator({
...
class AppVersionIndicator extends StatelessWidget {
  final Widget child;

  const AppVersionIndicator({Key? key, required this.child}) : super(key: key);

  Widget _buildVersionIndicator() {
    final version = js_util.getProperty(html.window, 'platformVersion').toString();
...
extension JuntoIterableExtension<T> on Iterable<T?> {
