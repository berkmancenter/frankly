...
import 'manipulate_discussion_thread_model.dart';
...
class ManipulateDiscussionThreadPresenter {
  ...
  })  : _discussionThreadsHelper = discussionThreadsHelper ?? DiscussionThreadsHelper(),
  ...
  Future<bool> addNewDiscussionThread() async {
    final juntoId = _model.juntoProvider.juntoId;
    final documentId = _firestoreDiscussionThreadsService.getNewDocumentId(juntoId);
    final discussionThread = await _discussionThreadsHelper.addNewDiscussionThread(
    ...
    await _firestoreDiscussionThreadsService.addNewDiscussionThread(juntoId, discussionThread);
  ...
  Future<bool> updateDiscussionThread() async {
    final discussionThread = await _discussionThreadsHelper.updateDiscussionThread(
      existingDiscussionThread: _model.existingDiscussionThread,
      discussionThreadContent: _model.content,
      pickedImageUrl: _model.pickedImageUrl,
      generalHelperService: _mediaHelperService,
      onError: (message) => _view.showMessage(message, toastType: ToastType.failed),
    );
    ...
    await _firestoreDiscussionThreadsService.updateDiscussionThread(juntoId, discussionThread);
  ...
  String getJuntoDisplayId() {
    return _model.juntoProvider.displayId;
  }
