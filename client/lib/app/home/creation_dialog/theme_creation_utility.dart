...
class ThemeUtils {
  ...
  static Color lightColorFromTheme(int theme) => presetColorThemes[theme].lightColor;

  static Color darkColorFromTheme(int theme) => presetColorThemes[theme].darkColor;

  static String convertToHexString(Color color) => color.toString().substring(10, 16);
  ...
  static bool isColorValid(String? color) =>
      color != null && color.length == 6 && RegExp(r'^[0-9a-fA-F]*$').hasMatch(color);

  static bool isColorComboValid(String? lightColorString, String? darkColorString) {
    bool colorStringsValid = isColorValid(lightColorString) && isColorValid(darkColorString);
    ...
    bool lightDarkCorrect =
        isFirstColorLighter(parseColor(lightColorString)!, parseColor(darkColorString)!);

    final darkColorIsDarkEnough = isContrastRatioValid(darkColor, AppColor.gray6);
    final lightColorIsLightEnough = isContrastRatioValid(lightColor, AppColor.gray1);

    return validRatio && lightDarkCorrect && darkColorIsDarkEnough && lightColorIsLightEnough;
  }
