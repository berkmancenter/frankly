...
class ThemePreview extends StatelessWidget {
  ...
  Color get _selectedDarkColor =>
      selectedTheme?.darkColor ?? ThemeUtils.parseColor(darkColorString) ?? AppColor.darkBlue;

  Color get _selectedLightColor =>
      selectedTheme?.lightColor ?? ThemeUtils.parseColor(lightColorString) ?? AppColor.gray6;

  Widget _buildPreviewContainer() {
    // Rounding up is not allowed, e.g. 4.47 != 4.5
    final contrastString =
        ((ThemeUtils.calculateContrastRatio(_selectedLightColor, _selectedDarkColor) * 10).floor() *
                .1)
            .toStringAsFixed(1);

    final showContrastRatio =
        ThemeUtils.isColorValid(lightColorString) || ThemeUtils.isColorValid(darkColorString);
            ...
            children: [
              Container(
                ...
                child: showContrastRatio
                    ? JuntoText(
                        contrastString,
                        style: AppTextStyle.eyebrow.copyWith(color: _selectedLightColor),
                      )
                    : null,
