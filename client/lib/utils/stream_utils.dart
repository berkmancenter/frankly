...
/// This class wraps JuntoStreamBuilder, and manages the lifecycle of the stream including
/// rebuilding if any parameter within `keys` changes
class JuntoStreamGetterBuilder<T> extends HookWidget {
  ...
  const JuntoStreamGetterBuilder({
    ...
    this.height = 200,
    this.width,
    this.entryFrom = 'JuntoStreamGetterBuilder.build',
    this.showLoading = true,
  }) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return JuntoStreamBuilder<T>(
...
class _BehaviorSubjectWrapperHook<T> extends Hook<BehaviorSubjectWrapper<T>> {
  ...
  @override
  _BehaviorSubjectWrapperHookState<T> createState() => _BehaviorSubjectWrapperHookState<T>();
}

class _BehaviorSubjectWrapperHookState<T>
    extends HookState<BehaviorSubjectWrapper<T>, _BehaviorSubjectWrapperHook<T>> {
  late final BehaviorSubjectWrapper<T> value = wrapInBehaviorSubject(hook.valueBuilder());
...
void useStreamListener<T>(
    {required Stream<T> stream, required void Function(T) function, List<Object>? keys}) {
  useEffect(
    () {
...
extension StreamExtensions<T> on Stream<T?> {
  ...
  Stream<WithPreviousData<T>> withPrevious() {
    return startWith(null).bufferCount(2, 1).map((pair) => WithPreviousData(pair[0], pair[1]));
  }
}
