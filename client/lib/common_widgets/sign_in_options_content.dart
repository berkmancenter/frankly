...
class _SignInOptionsContentState extends State<SignInOptionsContent> {
  ...
  Future<void> _resetPassword() {
    return alertOnError(context, () async {
      ...
      await showAlert(context, 'Password reset link sent to ${_emailController.text}');
    });
  }
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        ...
        Column(
          children: [
            if (_emailSelected) ..._buildEmailWidgets() else ..._buildSignInProviderButtons(),
            SizedBox(height: 8),
  ...
  String _getTitleText() {
    if (widget.isPurchasingSubscription) {
      ...
      return 'New to Frankly?';
    } else {
      return 'Sign in to Frankly';
    }
  }
  ...
  Widget _buildTermsOfService() {
    return Text.rich(TextSpan(
      ...
      children: [
        TextSpan(text: 'By signing in, registering, or using Frankly, I agree to be bound by the '),
        TextSpan(
          text: 'Frankly Terms of Service',
          recognizer: TapGestureRecognizer()..onTap = () => launch('https://frankly.org/terms'),
          style: TextStyle(
