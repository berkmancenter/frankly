...
class _UpgradeTooltipContents extends StatefulWidget {
  ...
  @override
  State<_UpgradeTooltipContents> createState() => _UpgradeTooltipContentsState();
}
...
class _UpgradeTooltipContentsState extends State<_UpgradeTooltipContents> {
  ...
  void _launchBilling() {
    ...
    launch('https://frankly.org/pricing', targetIsSelf: true);
  }
...
class _TooltipAnchorState extends State<TooltipAnchor> {
  ...
  void _calculateOffset() {
    WidgetsBinding.instance?.addPostFrameCallback((_) {
      ...
      final renderBox = _positionKey.currentContext?.findRenderObject() as RenderBox;
      final toolTipCenter = renderBox.localToGlobal(renderBox.paintBounds.center);
      setState(() => _offset = ((screenWidth - toolTipCenter.dx) - 156).clamp(-126, 0));
    });
  }
...
class _TooltipContainerState extends State<_TooltipContainer> {
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        Column(
          verticalDirection: widget.isBelowIcon ? VerticalDirection.up : VerticalDirection.down,
