...
class JuntoTextField extends StatefulWidget {
  ...
  const JuntoTextField({
    ...
    this.contentPadding = const EdgeInsets.symmetric(vertical: 12, horizontal: 10),
    this.borderRadius = 5,
  ...
  @override
  _JuntoTextFieldState createState() => _JuntoTextFieldState();
}

class _JuntoTextFieldState extends State<JuntoTextField> {
    ...
    return widget.labelStyle ??
        AppTextStyle.bodySmall.copyWith(
          color: _focusNode.hasFocus
              ? (widget.useDarkMode ? AppColor.accentBlueLight : AppColor.accentBlue)
  ...
  TextStyle _buildTextStyle({bool isError = false}) {
    return widget.textStyle ??
        AppTextStyle.body.copyWith(
          color: isError
              ? (widget.useDarkMode ? AppColor.redDarkMode : AppColor.redLightMode)
              : (widget.useDarkMode ? AppColor.white : AppColor.gray2),
        );
  }

  TextStyle _buildOptionalTextStyle() {
    return widget.optionalTextStyle ?? AppTextStyle.bodySmall.copyWith(color: AppColor.gray3);
  }
        ...
        child: RawKeyboardListener(
          ...
          child: Stack(
            children: [
                ...
                buildCounter: (_,
                    ...
                    maxLength != null && isFocused && !widget.hideCounter
                        ? Container(
                            ...
                            alignment: widget.counterAlignment ?? Alignment.centerRight,
                            child: Text(
                              currentLength.toString() + '/' + maxLength.toString(),
                              style: widget.counterStyle ?? AppTextStyle.bodySmall,
                            ),
                          )
                        : null,
              ...
              if (widget.isOptional && !_focusNode.hasFocus && _controller.text.isEmpty)
                Align(
  ...
  EdgeInsetsGeometry _buildOptionalPadding() {
    return widget.optionalPadding ?? const EdgeInsets.only(top: 16, right: 12.0);
  }
}
