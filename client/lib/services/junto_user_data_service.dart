...
class JuntoUserDataService with ChangeNotifier {
  ...
  final _userCommunities = BehaviorSubject<List<Junto>>();
  Stream<List<Junto>> get userCommunities => _userCommunities.stream;
    ...
    userService.addListener(() {
      if (_currentUserId != userService.currentUserId) {
        ...
        if (_currentUserId != null) {
          ...
          final userStatusDatabaseRef = FirebaseDatabase.instance.ref('/status/$_currentUserId');
          ...
          FirebaseDatabase.instance.ref('.info/connected').onValue.listen((event) {
            ...
            userStatusDatabaseRef.onDisconnect().set(isOfflineForDatabase).then((_) {
              ...
              debugPrint('OnDisconnect function activated. User is logged as being online.');
  ...
  void _loadUserData() {
    ...
    if (userId == null || !userService.isSignedIn) {
      ...
      _membershipsSubscription = _memberships.stream.listen((membershipList) async {
        final juntoIds = membershipList
            // Filter out nulls additionally to make sure list is with only String values
            .where((membership) => membership.isMember)
            .map((membership) => membership.juntoId)
            .toList();
        final juntoDocs = await firestoreDatabase.getJuntoDocuments(juntoIds);
        _userCommunities.add(juntoDocs.withoutNulls.toList());
        notifyListeners();
      });
    }
  ...
  Membership getMembership(String juntoId) {
    ...
    return _memberships.stream.valueOrNull?.firstWhere(
          (membership) => membership.juntoId == juntoId,
          orElse: () => membership,
        ) ??
        membership;
  }

  bool isMember({required String juntoId}) => getMembership(juntoId).isMember;

  /// Make membership change
  Future<void> changeJuntoMembership({
    required String juntoId,
    required String userId,
    required MembershipStatus newStatus,
    bool allowMemberDowngrade = true,
  }) async {
    final _invisible = userService.userIsSuperAdmin && userId == userService.currentUserId;
    ...
    if (!allowMemberDowngrade &&
        [MembershipStatus.member, MembershipStatus.attendee].contains(newStatus) &&
        (membership?.isMember ?? false)) {
      skipUpdate = true;
    }

    if (!skipUpdate) {
      await cloudFunctionsService.updateMembership(UpdateMembershipRequest(
        juntoId: juntoId,
        status: newStatus,
        userId: userId,
        invisible: _invisible ? _invisible : null,
      ));

      final wasMember = membership?.isMember ?? false;
      final isNowMember = newStatus.isMember;
      if (!wasMember && isNowMember) {
        analytics.logEvent(AnalyticsJoinJuntoEvent(juntoId: juntoId));
      } else if (wasMember && !isNowMember) {
        analytics.logEvent(AnalyticsLeaveJuntoEvent(juntoId: juntoId));
      }
    }
  }

  /// Trigger UI to change membership
  Future<void> requestChangeJuntoMembership({
    ...
    return guardSignedIn(() async {
      ...
      if (juntoUserDataService.getMembership(junto.id).isAdmin && !join) {
      ...
      if (!join) {
        final unsubscribe = await ConfirmDialog(
          title: 'Unfollow',
          mainText: 'Are you sure you want to unsubscribe from ${junto.name}?',
          confirmText: 'Yes',
          cancelText: 'No',
        ).show();
        if (unsubscribe != true) {
