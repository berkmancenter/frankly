...
class ClockService {
  ...
  Future<void> _loadTime() async {
    ...
    final serverTimestamp = await cloudFunctionsService.getServerTimestamp();
    ...
    final serverAppStartTimeMidpoint = serverTimestamp.subtract(Duration(
    ...
    loggingService.log('ServerAppStartTimeMidpoint: $serverAppStartTimeMidpoint');

    final offsetFromServer = serverAppStartTimeMidpoint.difference(deviceAppStartTime);

    final useServerTime = flightTime < _maxAllowedFlightTime.inMilliseconds &&
        offsetFromServer.abs() > _systemClockAllowedOffset;
    if (useServerTime) {
      loggingService.log('Replacing device time with time retrieved from server');
