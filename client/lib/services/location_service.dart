...
class QueryParametersService {
    ...
    if (queryParamsString != null) {
      return swallowErrorsSync<Map<String, String>?>(() {
        ...
        if (queryParams is Map && queryParams.isNotEmpty) {
          return queryParams.map((key, value) => MapEntry(key.toString(), value.toString()));
        }
        return null;
      });
    }
  ...
  void addQueryParameters(Map<String, String> parameters) {
    _queryParameters = (mostRecentQueryParameters ?? {})..addEntries(parameters.entries);
    sharedPreferencesService.setLastQueryParameters(jsonEncode(_queryParameters));
  }
}
