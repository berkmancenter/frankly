...
class FirestoreUserService {
  ...
  DocumentReference<Map<String, dynamic>> publicUserReference({required String userId}) =>
  ...
  Future<PublicUserInfo?> getPublicUserByAgoraId({required int agoraId}) async {
    final querySnapshot = await firestoreDatabase.firestore.collection(publicUser).where('agoraId', isEqualTo: agoraId).get();
  ...
  Future<PublicUserInfo> getOrCreatePublicUserInfo({
    ...
    return await firestoreDatabase.firestore.runTransaction((transaction) async {
  ...
  Future<void> updatePublicUser({
    ...
    await docRef.set(jsonSubset(keys, toFirestoreJson(userInfo.toJson())), SetOptions(merge: true));
  }

  static Future<PublicUserInfo> _convertPublicUserInfoAsync(
      DocumentSnapshot<Map<String, dynamic>> doc) async {
    final publicUser =
        await compute<Map<String, dynamic>, PublicUserInfo>(_convertPublicUserInfo, doc.data()!);
    return publicUser;
  }
