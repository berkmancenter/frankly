...
class FirestoreChatService {
  ...
  CollectionReference<Map<String, dynamic>> _chatMessageCollection({
    ...
    return firestoreDatabase.firestore.collection('$parentPath/$chats/$chatId/messages');
  }
  ...
  BehaviorSubjectWrapper<List<ChatMessage>> chatMessagesStream({
    ...
    return wrapInBehaviorSubject(_chatMessageCollection(parentPath: parentPath, chatId: chatId)
  ...
  String generateNewChatMessageId(String parentPath, String chatId) {
    return _chatMessageCollection(parentPath: parentPath, chatId: chatId).doc().id;
  }
  ...
  Future<ChatMessage> createChatMessage({
    ...
    final messagesCollection = _chatMessageCollection(parentPath: parentPath, chatId: chatId);
    await messagesCollection.doc(chatMessage.id).set(toFirestoreJson(chatMessage.toJson()));
  ...
  Future<void> removeChatMessage({required ChatMessage chatMessage}) async {
    final messageRef =
        firestoreDatabase.firestore.doc('${chatMessage.collectionPath}/${chatMessage.id}');
    await messageRef.update(jsonSubset(
  ...
  static Future<List<ChatMessage>> _convertChatMessageListAsync(
    ...
    final chatMessages =
        await compute(_convertChatMessageList, docs.map((doc) => doc.data()).toList());
  ...
  static List<ChatMessage> _convertChatMessageList(List<Map<String, dynamic>> data) {
