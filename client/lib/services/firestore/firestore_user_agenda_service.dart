...
class FirestoreUserAgendaService {
  ...
  CollectionReference<Map<String, dynamic>> userSuggestionsRef({required String parentDoc}) {
  ...
  BehaviorSubjectWrapper<List<SuggestedAgendaItem>> suggestedAgendaItemsStream({
    ...
    return wrapInBehaviorSubject(userSuggestionsCollection
        .snapshots()
        .map((s) => s.docs)
        .asyncMap(_convertSuggestedAgendaItemsListAsync));
  }

  static Future<List<SuggestedAgendaItem>> _convertSuggestedAgendaItemsListAsync(
      List<QueryDocumentSnapshot<Map<String, dynamic>>> docs) async {
    final suggestedAgendaItems =
        await Future.wait(docs.map((doc) => compute(_convertSuggestedAgendaItem, doc.data())));
    ...
    suggestedAgendaItems.sort((a, b) {
      ...
      if (aScore == bScore) {
        return (a.createdDate ?? clockService.now()).compareTo(b.createdDate ?? clockService.now());
      }
  ...
  static SuggestedAgendaItem _convertSuggestedAgendaItem(Map<String, dynamic> data) =>
  ...
  Future<void> voteOnSuggestedAgendaItem({
    ...
    return firestoreDatabase.firestore.runTransaction((transaction) async {
      final agendaItemRef = userSuggestionsRef(parentDoc: parentDoc).doc(itemId);

      final agendaItem =
          SuggestedAgendaItem.fromJson(fromFirestoreJson((await transaction.get(agendaItemRef)).data()!));
      ...
      transaction.update(
          agendaItemRef,
          jsonSubset(
            [SuggestedAgendaItem.kFieldUpvotedUserIds, SuggestedAgendaItem.kFieldDownvotedUserIds],
