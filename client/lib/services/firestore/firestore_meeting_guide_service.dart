...
class FirestoreMeetingGuideService {
  ...
  Stream<List<ParticipantAgendaItemDetails>> participantAgendaItemDetailsStream({
    required String liveMeetingPath,
    required String agendaItemId,
  }) {
  ...
  Future<void> addWordCloudResponse({
    ...
    await firestoreDatabase.firestore.doc(documentPath).set(
        jsonSubset([
          ...
          ParticipantAgendaItemDetails.kFieldWordCloudResponses: FieldValue.arrayUnion([response]),
        }),
        SetOptions(merge: true));
  }
  ...
  Future<void> removeWordCloudResponse({
    ...
    await firestoreDatabase.firestore.doc(documentPath).set(
        jsonSubset([
          ...
          ParticipantAgendaItemDetails.kFieldUserId: userId,
          ParticipantAgendaItemDetails.kFieldAgendaItemId: agendaItemId,
          ParticipantAgendaItemDetails.kFieldWordCloudResponses: FieldValue.arrayRemove([response]),
        }),
        SetOptions(merge: true));
  }
  ...
  Future<void> addUserSuggestion({
    ...
    loggingService.log('FirestoreMeetingGuideService.addUserSuggestion: Data: $dataMap');

    await firestoreDatabase.firestore.doc(documentPath).set(toFirestoreJson(dataMap), SetOptions(merge: true));
  }
  ...
  Future<void> removeUserSuggestion({
    ...
    loggingService.log('FirestoreMeetingGuideService.removeUserSuggestion: Data: $dataMap');

    await firestoreDatabase.firestore.doc(documentPath).set(dataMap, SetOptions(merge: true));
  }
  ...
  Future<void> toggleHandRaise({
    ...
    loggingService.log('FirestoreMeetingGuideService.toggleHandRaise: Data: $updateMap');

    // handRaisedTime is set to serverTimestamp unless we reset to null.
    if (isHandRaised) {
      updateMap[ParticipantAgendaItemDetails.kFieldHandRaisedTime] = FieldValue.serverTimestamp();
    } else {
      updateMap[ParticipantAgendaItemDetails.kFieldHandRaisedTime] = null;
    }

    await firestoreDatabase.firestore.doc(documentPath).set(toFirestoreJson(updateMap), SetOptions(merge: true));
  }
  ...
  Future<void> updateVideoPosition({
    ...
    await firestoreDatabase.firestore.doc(documentPath).set(
        jsonSubset(
            ...
            toFirestoreJson(ParticipantAgendaItemDetails(
              userId: userId,
              agendaItemId: agendaItemId,
              meetingId: meetingId,
  ...
  static Future<List<ParticipantAgendaItemDetails>> _convertParticipantAgendaItemDetailsListAsync(
    List<QueryDocumentSnapshot<Map<String, dynamic>>> docs,
  ) async {
    return Future.wait(docs.map(
      (doc) => compute(_convertSuggestedAgendaItem, doc.data()),
    ));
  }

  static ParticipantAgendaItemDetails _convertSuggestedAgendaItem(Map<String, dynamic> data) {
    print('item details');
    print(data);
