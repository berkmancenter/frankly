...
class StripeUtil {
  ...
  Future<String> getOrCreateCustomerStripeId({required String uid}) async {
    ...
    final Map<String, String> params = {'email': user.email, 'metadata[uid]': user.uid};
    ...
    await stripeUserDataDoc.setData(
        DocumentData.fromMap({'stripeId': stripeId}), SetOptions(merge: true));
  ...
  Future<Map<String, dynamic>> post({
    ...
    final response = await http.post(Uri.parse('https://api.stripe.com/v1$path'),
  ...
  Future<Map<String, dynamic>> delete({required String path, String? connectedAccount}) async {
    final headers = {
      'Authorization': 'Bearer $_secretKey',
      if (connectedAccount != null) 'Stripe-Account': connectedAccount,
    };
    ...
    final response =
        await http.delete(Uri.parse('https://api.stripe.com/v1$path'), headers: headers);
    if (response.statusCode < 200 || response.statusCode > 299) {
      print('Stripe POST error:');
      print(response.body);
      throw HttpsError(HttpsError.internal, 'Internal error.', null);
  ...
  Future<Map<String, dynamic>> get({
    ...
    final response = await http.get(Uri.parse('https://api.stripe.com/v1$path'), headers: headers);
    if (response.statusCode < 200 || response.statusCode > 299) {
