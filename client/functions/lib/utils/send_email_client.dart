...
class SendEmailClient {
  Future<void> sendEmail(SendGridEmail email, {Transaction? transaction}) async {
    final newDocument = firestore.collection('sendgridmail').document();

    final newData = DocumentData.fromMap(firestoreUtils.toFirestoreJson(email.toJson()));
    if (transaction != null) {
  ...
  Future<void> sendEmails(List<SendGridEmail> emails) async {
    await Future.wait(partition(emails, 500).map((sublist) {
      final batch = firestore.batch();
      sublist.map((email) {
        final doc = firestore.collection('sendgridmail').document();
        batch.setData(doc, DocumentData.fromMap(firestoreUtils.toFirestoreJson(email.toJson())));
      }).toList();
      return batch.commit();
    }).toList());
  }
