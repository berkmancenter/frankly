...
class SubscriptionPlanUtil {
  ...
  /// Given a junto, return its list of capabilities (governed by its owners subscription level and
  ...
  Future<PlanCapabilityList> calculateCapabilities(
    ...
    if (requesterUserId != null) {
      final membershipDoc = 'memberships/$requesterUserId/junto-membership/$juntoId';
    ...
    final overridePlan = await _getOverridePlan(juntoId: juntoId);

    final subscriptionPlan = await _getSubscriptionPlan(juntoId: juntoId);
  ...
  /// Check if this junto is covered by a partner-agreements record that specifies a plan override.
  /// If so, just return the capabilities for that plan type.
  Future<PlanCapabilityList?> _getOverridePlan({required String juntoId}) async {
    final agreementDocs = await firestore
        .collection('partner-agreements')
        .where(PartnerAgreement.kFieldJuntoId, isEqualTo: juntoId)
        .limit(1)
        .get();

    if (agreementDocs.documents.isNotEmpty) {
      PartnerAgreement agreement =
          PartnerAgreement.fromJson(firestoreUtils.fromFirestoreJson(agreementDocs.documents.first.data.toMap()));
  ...
  Future<PlanCapabilityList?> _getSubscriptionPlan({required String juntoId}) async {
    // Get active subscriptions for juntoId
    ...
    final activeSubscriptions = await subscriptionCollection
        .where(BillingSubscription.kFieldActiveUntil, isGreaterThan: (DateTime.now().toUtc()))
        .where(BillingSubscription.kFieldAppliedJuntoId, isEqualTo: juntoId)
        .get();

    if (activeSubscriptions.isNotEmpty) {
      // Return combined capability list
      final subscriptions = activeSubscriptions.documents
          .map((doc) => BillingSubscription.fromJson(firestoreUtils.fromFirestoreJson(doc.data.toMap())));
      final types = subscriptions.map((subscription) => subscription.type).toSet();
  ...
  Future<PlanCapabilityList> _getCapabilitiesForType({required String type}) async {
    ...
    if (matchingCapabilities.documents.isNotEmpty) {
      return PlanCapabilityList.fromJson(firestoreUtils.fromFirestoreJson(matchingCapabilities.documents.first.data.toMap()));
    } else {
  ...
  PlanCapabilityList _combineCapabilities(PlanCapabilityList a, PlanCapabilityList b) {
    return PlanCapabilityList(
      ...
      facilitatorCount: applyReducerWithNullable(max, a.facilitatorCount, b.facilitatorCount),
      takeRate: applyReducerWithNullable(min, a.takeRate, b.takeRate),
      hasSmartMatching:
          applyReducerWithNullable(orOperator, a.hasSmartMatching, b.hasSmartMatching),
      hasLivestreams: applyReducerWithNullable(orOperator, a.hasLivestreams, b.hasLivestreams),
      hasCustomUrls: applyReducerWithNullable(orOperator, a.hasCustomUrls, b.hasCustomUrls),
      hasAdvancedBranding:
          applyReducerWithNullable(orOperator, a.hasAdvancedBranding, b.hasAdvancedBranding),
      hasBasicAnalytics:
          applyReducerWithNullable(orOperator, a.hasBasicAnalytics, b.hasBasicAnalytics),
      hasCustomAnalytics:
          applyReducerWithNullable(orOperator, a.hasCustomAnalytics, b.hasCustomAnalytics),
      hasIntegrations: applyReducerWithNullable(orOperator, a.hasIntegrations, b.hasIntegrations),
      hasPrePost: applyReducerWithNullable(orOperator, a.hasPrePost, b.hasPrePost),
    );
  }
