...
AgoraTokenModule get agoraModule => _agoraModule ??= require('agora-token') as AgoraTokenModule;
...
final _agoraAppCertificate = functions.config.get('agora.app_certificate') as String;

final _agoraStorageBucketName = functions.config.get('agora.storage_bucket_name') as String;
final _agoraStorageAccessKey = functions.config.get('agora.storage_access_key') as String;
final _agoraStorageSecretKey = functions.config.get('agora.storage_secret_key') as String;
...
class AgoraUtils {
  ...
  Map<String, String> _getAuthHeaders() {
    ...
    return {'Authorization': authorizationField, 'Content-Type': 'application/json'};
  }
  ...
  Future<String> _acquireResourceId({required String roomId}) async {
    ...
    final result = await http.post(
      Uri.parse('https://api.agora.io/v1/apps/$_agoraAppId/cloud_recording/acquire'),
  ...
  Future<String> _startRecording({required String roomId, required String resourceId}) async {
    ...
    await _queryRecordingState(resourceId: resourceId, sid: convert.jsonDecode(result.body)['sid']);

    return result.body;
  }

  Future<void> _queryRecordingState({required String sid, required String resourceId}) async {
  ...
  Future<void> kickParticipant({required String roomId, required String userId}) async {
    final result = await http.post(
...
abstract class RtcTokenBuilderClient {
  external String buildTokenWithUid(
      String appId, String appCertificate, String channelName, int uid, int role, int token_expire);
}
