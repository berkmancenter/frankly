...
class NotificationsUtils {
  Future<void> sendJuntoNotifications({
    required String juntoId,
    String? creatorUserId,
    required bool Function(JuntoUserSettings) filterUsersBy,
    ...
    // first get members of this junto

    final juntoRef = await firestore.document('/junto/$juntoId').get();
    final juntoData =
        Junto.fromJson(firestoreUtils.fromFirestoreJson(juntoRef.data.toMap()));
    ...
    final userSettingsFutures = userIds.map((userId) async {
      final snapshot = await firestore
          .document('/privateUserData/$userId/juntoUserSettings/$juntoId')
          .get();
      final record = JuntoUserSettings.fromJson(
    ...
    final emails = users
        ...
        .map((user) => SendGridEmail(
            to: [user.email],
            from: '${juntoData.name} <$noReplyEmailAddr>',
  ...
  Future<void> sendEmailToDiscussionParticipants({
    required String juntoId,
    ...
    final juntoDoc =
        await firestore.collection('junto').document(juntoId).get();
    final junto =
        Junto.fromJson(firestoreUtils.fromFirestoreJson(juntoDoc.data.toMap()));

    // Get participant ids which are in this discussion
    final discussionParticipantsSnapshots = await firestore
        .collection('junto')
        .document(juntoId)
        .collection('topics')
        .document(topic.id)
        ...
        .get();
    final discussionParticipants =
        discussionParticipantsSnapshots.documents.map(
      (e) => Participant.fromJson(
          firestoreUtils.fromFirestoreJson(e.data.toMap())),
    );

    final discussionParticipantIds = discussionParticipants
        // Only send messages to those that are active in the discussion
    ...
    final users = await firestoreUtils.getUsers(discussionParticipantIds);
    final usersWithoutEmail =
        users.where((u) => isNullOrEmpty(u.email)).toList();
    if (usersWithoutEmail.isNotEmpty) {
    ...
    print('Sending discussion message email to: ${emails.map((e) => e.to)}');

    await sendEmailClient.sendEmails(emails);
  }

  Future<void> sendDiscussionEndedEmail({
    required String juntoId,
    required Discussion discussion,
    required DiscussionEmailType emailType,
    required List<String> userIds,
    required SendGridEmailMessage Function(
            Junto junto, admin_interop.UserRecord user)
        generateMessage,
  }) async {
    // Get reference to the space which is used in the email
    final juntoDoc =
    ...
    final emailLogsCollection = firestore.collection(
      'junto/${discussion.juntoId}/topics/${discussion.topicId}/discussions/${discussion.id}/email-logs',
    );
    ...
    for (final user in lookedUpUsers) {
      ...
      await sendEmailClient.sendEmail(
        SendGridEmail(
          to: [user.email],
          from: '${junto.name} <$noReplyEmailAddr>',
          message: generateMessage(junto, user),
        ),
      );
