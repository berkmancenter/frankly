...
class CheckAdvanceMeetingGuide extends OnCallMethod<CheckAdvanceMeetingGuideRequest> {
  CheckAdvanceMeetingGuide()
      : super('CheckAdvanceMeetingGuide',
  ...
  Future<void> _markReady({
    ...
    final docData = DocumentData.fromMap(jsonSubset(
        [
        ...
        firestoreUtils.toFirestoreJson(ParticipantAgendaItemDetails(
  ...
  @override
  Future<void> action(CheckAdvanceMeetingGuideRequest request, CallableContext context) async {
    // Look up discussion
    ...
    final activeLiveMeetingPath = isBreakout ? breakoutLiveMeetingPath : liveMeetingPath;
  ...
  Future<void> _checkAdvanceMeetingGuide({
    ...
    if (liveMeeting.events.any((e) => e.event == LiveMeetingEventType.finishMeeting)) {
    ...
    final currentAgendaItemId = _getCurrentAgendaItemId(discussion, liveMeeting);
    print('current agenda item: $currentAgendaItemId');

    // Determine who is present
    DocumentQuery participantsQuery =
        firestore.collection('${request.discussionPath}/discussion-participants');
    if (isBreakout) {
    ...
    final registeredParticipants = participantsSnapshot.documents
        .map((doc) => Participant.fromJson(firestoreUtils.fromFirestoreJson(doc.data?.toMap() ?? {})))
        .where((participant) => participant.status == ParticipantStatus.active)
        .toList();
    final registeredParticipantIds = registeredParticipants.map((p) => p.id).toSet();
    ...
    final agendaItemParticipantDetailsPath = '$liveMeetingPath/participant-agenda-item-details/'
    ...
    final agendaItemParticipantDetails = agendaItemParticipantDetailsDocs.documents
        .map((doc) => ParticipantAgendaItemDetails.fromJson(firestoreUtils.fromFirestoreJson(doc.data.toMap())))
        .toList();

    final readyToMoveOnIds = <String>{
      ...agendaItemParticipantDetails
          .where((a) => (a.readyToAdvance ?? false) && presentParticipantIds.contains(a.userId))
          .map((p) => p.userId ?? ''),
      if (request.userReadyAgendaId == currentAgendaItemId && !isNullOrEmpty(userId)) userId,
    };
  ...
  String _getCurrentAgendaItemId(Discussion discussion, LiveMeeting liveMeeting) {
    final events =
        liveMeeting.events.where((e) => LiveMeetingEventType.agendaItemStarted == e.event).toList();
  ...
  Future<void> _advanceMeetingGuide({
    ...
    await firestore.runTransaction((transaction) async {
      ...
      final newCurrentAgendaItemId = _getCurrentAgendaItemId(discussion, liveMeeting);
      ...
      final agendaItemIndex = agendaItems.indexWhere((a) => a.id == currentAgendaItemId);

      var nextAgendaItem =
          agendaItemIndex + 1 < agendaItems.length ? agendaItems[agendaItemIndex + 1] : null;
      ...
      if (currentAgendaItemId == startMeetingAgendaItemId && parentLiveMeetingPath != null) {
        ...
        final parentAgendaItemId = _getCurrentAgendaItemId(discussion, parentLiveMeeting);
        final AgendaItem? parentAgendaItem =
            agendaItems.where((item) => item.id == parentAgendaItemId).firstOrNull;
      ...
      final allTimingEvents = liveMeeting.events.where((e) => [
            LiveMeetingEventType.agendaItemStarted,
            LiveMeetingEventType.finishMeeting
          ].contains(e.event));
