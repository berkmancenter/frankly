...
class ToggleLikeDislikeOnMeetingUserSuggestion
  ...
  @override
  Future<void> action(ParticipantAgendaItemDetailsMeta request, CallableContext context) async {
    await firestore.runTransaction((transaction) async {
      final agendaItemDetailsDocRef = firestore.document(request.documentPath);
      final agendaItemDetailsSnap = await transaction.get(agendaItemDetailsDocRef);
      if (!agendaItemDetailsSnap.exists) {
        print('agendaItemDetailsSnap from path ${request.documentPath} does not exist');
        return;
      }
      ...
      try {
        agendaItemDetails = ParticipantAgendaItemDetails.fromJson(firestoreUtils.fromFirestoreJson(agendaItemDetailsDataMap));
      } catch (e) {
        print('Cannot parse participant agenda item details: $agendaItemDetailsDataMap');
        return;
      }

      final userSuggestion = agendaItemDetails.suggestions.where(
      ...
      print('Toggle: ${request.likeType} for user suggestion ${userSuggestion.id}');
      ...
      switch (request.likeType) {
        case LikeType.like:
          ...
          userSuggestion.dislikedByIds.removeWhere((element) => element == voterId);
          break;
        case LikeType.neutral:
          userSuggestion.likedByIds.removeWhere((element) => element == voterId);
          userSuggestion.dislikedByIds.removeWhere((element) => element == voterId);
          break;
        case LikeType.dislike:
          userSuggestion.likedByIds.removeWhere((element) => element == voterId);
          if (!userSuggestion.dislikedByIds.any((element) => element == voterId)) {
      ...
      transaction.update(agendaItemDetailsDocRef, UpdateData.fromMap(firestoreUtils.toFirestoreJson(updateDataMap)));
    });
  }
}
