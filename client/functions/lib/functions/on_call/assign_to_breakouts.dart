...
class AssignToBreakouts {
  ...
  Future<List<BreakoutRoom>> _assignBreakoutsBasedOnTargetSize({
    ...
    for (final id in presentParticipantIds) {
      memberBuckets.add(participantMembershipLookup[id] ?? MembershipStatus.nonmember, id);
    }

    print('Get all grouped ids in order');
    // Print out all values from the multimap in order of the MembershipStatus Enum
    final groupedIds = [for (final status in MembershipStatus.values) memberBuckets[status]]
        .expand((list) => list)
        .toList();
    ...
    if (targetParticipantsPerRoom == 0) {
      ...
      numRooms = math.max(
          1, (presentParticipantIds.length.toDouble() / targetParticipantsPerRoom).round());
    }
  ...
  String? _getJoinParametersOrNull(Participant participant, Discussion discussion) {
    String? surveyAnswers;
    final allQuestionsAnswered =
        participant.breakoutRoomSurveyQuestions.every((q) => q.answerOptionId.isNotEmpty);
    if (allQuestionsAnswered) {
      final surveyResponses = participant.breakoutRoomSurveyQuestions.map((q) {
        final answerId = q.answerOptionId;
        final answerIndex =
            q.answers.indexWhere((answer) => answer.options.any((option) => option.id == answerId));
    ...
    if ((surveyAnswers == null || surveyAnswers.isEmpty) &&
        joinParameters['discussionId'] == discussion.id) {
  ...
  void _normalizeParticipantSurveyResponses(
      Map<String, String> participantSurveyResponsesLookup, Discussion discussion) {
    int? numberOfQuestions = discussion.breakoutRoomDefinition?.breakoutQuestions.length;

    final hasQuestions = numberOfQuestions != null && numberOfQuestions > 0;
    if (!hasQuestions && participantSurveyResponsesLookup.values.isNotEmpty) {
      ...
      numberOfQuestions =
          participantSurveyResponsesLookup.values.map((s) => s.length).reduce(math.max);
    }
    ...
    participantSurveyResponsesLookup.updateAll((key, value) {
      if (value.length > nonNullNumberOfQuestions) {
        ...
        final padding = [for (int i = 0; i < paddingLength; i++) random.nextBool() ? 0 : 1].join();
  ...
  Future<List<BreakoutRoom>> _assignBreakoutsForSmartMatch({
    ...
    final prematchedIds = prematches.values.expand((p) => p).map((p) => p.id).toSet();
    ...
    for (final participant in unmatchedParticipants) {
      final joinParameters = _getJoinParametersOrNull(participant, discussion);
      if (joinParameters != null) {
        participantSurveyResponsesLookup[participant.id] = joinParameters;
      }
    }
    _normalizeParticipantSurveyResponses(participantSurveyResponsesLookup, discussion);
    ...
    if (targetParticipantsPerRoom <= 2 || participantSurveyResponsesLookup.length <= 2) {
      ...
      final adjustedTargetParticipants = calculateAdjustedTargetParticipants(
          participantSurveyResponsesLookup.length, targetParticipantsPerRoom);
    ...
    if (smartMatches.isNotEmpty && smartMatches.last.length < targetParticipantsPerRoom) {
    ...
    print('unmatched: ${unmatchedParticipants.length}');
    final leftoverMatches =
        partition(unmatchedParticipants.map((e) => e.id), targetParticipantsPerRoom);
    ...
    final breakoutMatchIdsToRecord = discussion.breakoutMatchIdsToRecord.toSet();
    ...
    return [
      for (i = 0; i < prematchEntries.length; i++)
        BreakoutRoom(
          ...
          originalParticipantIdsAssignment: prematchEntries[i].value.map((p) => p.id).toList(),
  ...
  static int calculateAdjustedTargetParticipants(
      int numParticipants, int targetParticipantsPerRoom) {
    if (targetParticipantsPerRoom < 4) {
      return targetParticipantsPerRoom;
    }
    final numRoomsDouble = numParticipants / targetParticipantsPerRoom.toDouble();
  ...
  Future<void> writeDocumentsToCollection({
    ...
    return Future.wait(partition(rooms, 249).map((sublist) {
      final batch = firestore.batch();
      for (final room in sublist) {
        final roomDocumentRef = breakoutSessionCollection.document(room.roomId);
        batch.setData(
            roomDocumentRef, DocumentData.fromMap(firestoreUtils.toFirestoreJson(room.toJson())));
        if (firstAgendaItemId != null) {
          final liveMeetingDoc = roomDocumentRef.collection('live-meetings').document(room.roomId);
          batch.setData(
              liveMeetingDoc,
  ...
  Future<Iterable<DocumentSnapshot>> getParticipantSnapshots({
    ...
    for (int i = 0; i <= characterFilters.length; i++) {
      ...
      if (previous != null) {
        query = query.where(Participant.kFieldId, isGreaterThanOrEqualTo: previous);
      }
      if (next != null) {
    ...
    print(Map.fromEntries([
      for (int i = 0; i < allSnapshots.length; i++) MapEntry(i, allSnapshots[i].documents!.length),
    ]));
  ...
  Future<bool> _markProcessingAssignmentsIfAvailable({
    ...
    return await firestore.runTransaction((transaction) async {
      ...
      final liveMeeting =
          LiveMeeting.fromJson(firestoreUtils.fromFirestoreJson(document.data.toMap()));
      final currentBreakoutSession = liveMeeting.currentBreakoutSession;

      final isNewSession = currentBreakoutSession?.breakoutRoomSessionId != breakoutSessionId;

      final isPendingOrInactive = [BreakoutRoomStatus.pending, BreakoutRoomStatus.inactive]
          .contains(currentBreakoutSession?.breakoutRoomStatus);

      final statusUpdateTime = currentBreakoutSession?.statusUpdatedTime ?? DateTime.now();
      final isProcessingCanceled =
  ...
  Future<void> _processAssignments({
    ...
    final participantSnapshots = (await getParticipantSnapshots(
      participantsPath: '${discussion.fullPath}/discussion-participants',
    ))
        .toList();
    ...
    presentParticipants = presentParticipants.where((participant) {
      final assignAllPresent = discussion.discussionType == DiscussionType.hosted;
      ...
      final userIsAvailable =
          (!assignAllPresent && participant.availableForBreakoutSessionId == breakoutSessionId) ||
    ...
    final breakoutRoomsSessionDoc =
        firestore.document('$liveMeetingPath/breakout-room-sessions/$breakoutSessionId');
    final breakoutRoomsCollection = breakoutRoomsSessionDoc.collection('breakout-rooms');
    ...
    if (assignmentMethod == BreakoutAssignmentMethod.targetPerRoom) {
      ...
      throw Exception('Unknown breakout rooms request assignment method: $assignmentMethod');
    }
    ...
    if (currentLiveMeeting.currentBreakoutSession?.breakoutRoomStatus !=
        ...
        currentLiveMeeting.currentBreakoutSession?.processingId != processingId) {
      profile('No longer processing or processingId doesnt match. Returning without writing rooms');
      profile('status: ${currentLiveMeeting.currentBreakoutSession?.breakoutRoomStatus}');
      profile('processingId: ${currentLiveMeeting.currentBreakoutSession?.processingId}');
      return;
    }
    ...
    await breakoutRoomsSessionDoc.setData(
        DocumentData.fromMap(firestoreUtils.toFirestoreJson(breakoutRoomSession.toJson())));

    await firestore.document(liveMeetingPath).setData(
        DocumentData.fromMap(firestoreUtils.toFirestoreJson({
          LiveMeeting.kFieldCurrentBreakoutSession: breakoutRoomSession.toJson(),
        })),
  ...
  Future<void> assignToBreakouts({
    ...
    profile('updating breakout room to assigning with processingID: $processingId');
