...
class GetBreakoutRoomAssignment extends OnCallMethod<GetBreakoutRoomAssignmentRequest> {
  GetBreakoutRoomAssignment()
      : super('GetBreakoutRoomAssignment',
            (jsonMap) => GetBreakoutRoomAssignmentRequest.fromJson(jsonMap));

  @override
  Future<Map<String, dynamic>> action(
      GetBreakoutRoomAssignmentRequest request, CallableContext context) async {
    ...
    final currentBreakoutRoom = await breakoutRoomsCollection
        .where(BreakoutRoom.kFieldParticipantIds, arrayContains: context?.authUid)
        .get();

    String? assignment;
    if (currentBreakoutRoom.documents.isNotEmpty) {
      assignment = BreakoutRoom.fromJson(firestoreUtils.fromFirestoreJson(currentBreakoutRoom.documents.first.data.toMap())).roomId;
    } else {
      final allBreakoutRoomDocs = await breakoutRoomsCollection.get();
      final allBreakoutRooms = allBreakoutRoomDocs.documents
          .map((doc) => BreakoutRoom.fromJson(firestoreUtils.fromFirestoreJson(doc.data.toMap())))
          .toList();

      assignment = await firestore.runTransaction((transaction) async {
        // Assign to waiting room if there is one.
        var assignedRoomIndex =
            allBreakoutRooms.indexWhere((br) => br.roomId == breakoutsWaitingRoomId);
        if (assignedRoomIndex < 0 && allBreakoutRooms.isNotEmpty) {
          // If no waiting room, then assign to a room with least participants
          final minBreakoutRoomCount =
              allBreakoutRooms.map((b) => b.participantIds.length).reduce((a, b) => a < b ? a : b);
          assignedRoomIndex =
              allBreakoutRooms.indexWhere((b) => b.participantIds.length == minBreakoutRoomCount);
        }
        ...
        if (assignedRoomIndex < 0) {
          ...
          final roomDoc =
              breakoutRoomsCollection.document(allBreakoutRooms[assignedRoomIndex].roomId);
          ...
          transaction.update(
              ...
              firestoreUtils.toFirestoreJson(assignedRoomLookup.copyWith(participantIds: [
