...
class OnDiscussionThreadComment extends OnFirestoreFunction<DiscussionThreadComment> {
  OnDiscussionThreadComment()
      : super(
          [
            AppFirestoreFunctionData(
                'DiscussionThreadCommentOnCreate', FirestoreEventType.onCreate),
            AppFirestoreFunctionData(
                'DiscussionThreadCommentOnDelete', FirestoreEventType.onDelete),
            AppFirestoreFunctionData(
                'DiscussionThreadCommentOnUpdate', FirestoreEventType.onUpdate),
          ],
          (snapshot) {
            final dataMap = snapshot.data!.toMap();
            return DiscussionThreadComment.fromJson(firestoreUtils.fromFirestoreJson(dataMap)).copyWith(id: snapshot.documentID);
          },
        );

  @override
  String get documentPath => firestoreHelper.getPathToDiscussionThreadCommentsTrigger();
  ...
  Future<void> onCreate(
    ...
    final pathToDiscussionThread = firestoreHelper.getPathToDiscussionThreadsDocument(
      juntoId: juntoId,
      discussionThreadId: discussionThreadId,
    );
    final discussionThreadSnap = await firestore.document(pathToDiscussionThread).get();
    final updateMap = {DiscussionThread.kFieldCommentCount: Firestore.fieldValues.increment(1)};
    print('ID: ${documentSnapshot.documentID}, Data: $updateMap');

    return discussionThreadSnap.reference.updateData(UpdateData.fromMap(updateMap));
  }
  ...
  Future<void> onUpdate(
    ...
    if (beforeIsDeleted != afterIsDeleted) {
      ...
      final Map<String, FieldValue> updateMap = {
        DiscussionThread.kFieldCommentCount: Firestore.fieldValues.increment(incrementValue)
      };

      print('Path: $pathToDiscussionThread, Data: $updateMap');
      await discussionThreadSnap.reference.updateData(UpdateData.fromMap(updateMap));
    }
  }
