...
class UpdatePresenceStatus implements JuntoCloudFunction {
  @override
  final String functionName = 'UpdatePresenceStatus';

  FutureOr<void> action(Change<DataSnapshot> change, EventContext context) async {
    try {
      final realtimeDatabasePresence = change.after.val() as Map<String, dynamic>;
      final afterPresenceUpdateTime =
          DateTime.fromMillisecondsSinceEpoch(realtimeDatabasePresence['last_changed']);
      ...
      final currentPresenceValue = currentPresenceSnapshot.val() as Map<String, dynamic>;
      final currentPresenceTimestamp =
          DateTime.fromMillisecondsSinceEpoch(currentPresenceValue['last_changed']);
      ...
      await _updateDiscussionStatusesToOffline(
  ...
  Future<void> _updateDiscussionStatusesToOffline({
    ...
    await Future.wait(documentsToUpdate.documents.map((doc) =>
        firestore.runTransaction((transaction) async {
          final liveParticipantData = await transaction.get(doc.reference);
          final participant = Participant.fromJson(firestoreUtils.fromFirestoreJson(liveParticipantData.data.toMap()));
          ...
          transaction.update(
              ...
              firestoreUtils.toFirestoreJson(participant
                      .copyWith(
                        isPresent: false,
                        currentBreakoutRoomId: '',
                      )
                      .toJson()
                    ..[Participant.kFieldLastUpdatedTime] = Timestamp.fromDateTime(updateTime)))));
        })));
  }
  ...
  void register(FirebaseFunctions functions) {
    functions[functionName] = functions
        .runWith(RuntimeOptions(timeoutSeconds: 60, memory: '1GB', minInstances: 0))
        .database
        .ref('/status/{uid}')
        .onUpdate(action);
  }
