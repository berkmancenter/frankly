...
import 'package:junto_functions/interop/stripe_client.dart';
import 'package:junto_functions/utils/analytics_util.dart';
...
import 'package:junto_models/firestore/payment_record.dart';
...
abstract class AbstractStripeWebhooks extends OnRequestMethod<JsonMap> {
  ...
  Future<void> handlePaymentIntentSucceeded(JsonMap request) async {
    ...
    if (metadata['type'] == 'one_time_donation') {
      final String juntoId = object['metadata']['juntoId'];
      ...
      await paymentRef.setData(DocumentData.fromMap(
          firestoreUtils.toFirestoreJson(paymentRecord.toJson())));

      analyticsUtil.logEvent(
        userId: authUid,
        event: AnalyticsDonateEvent(
            juntoId: juntoId, amount: amountInCents / 100.0),
      );
    } else {
  ...
  Future<void> handleAccountUpdated(JsonMap request) async {
    ...
    final agreementSnapshots = await firestore
        ...
        .where(PartnerAgreement.kFieldStripeConnectedAccountId,
            isEqualTo: object['id'])
        .get();
    ...
    for (final agreementDoc in agreementSnapshots.documents) {
      ...
      if (agreement.stripeConnectedAccountActive != hasTransfers) {
        ...
        await agreementDoc.reference.updateData(UpdateData.fromMap(jsonSubset(
  ...
  Future<void> _updateSubscriptionData(Map stripeSubscription) async {
    ...
    final appliedJuntoId =
        stripeSubscription['metadata']['appliedJuntoId'] as String?;
    final subscription = BillingSubscription(
        stripeSubscriptionId: stripeSubscriptionId,
        type: type,
        activeUntil: periodEndDateTime,
        appliedJuntoId: appliedJuntoId,
      ...
      DocumentData.fromMap(
        jsonSubset(
          [
            ...
            BillingSubscription.kFieldAppliedJuntoId,
