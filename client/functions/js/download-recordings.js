...
const downloadRecording = functions.runWith({ timeoutSeconds: 300 }).https.onRequest((req, res) => {
    cors(req, res, async () => {
        ...
        // Extract the discussionPath from the request body
        const { discussionPath } = req.body
        if (!discussionPath) {
            throw new functions.https.HttpsError('failed-precondition', 'discussionPath not found')
            return
        }

        // Fetch the discussion object
        const discussionDoc = await firestore.doc(discussionPath).get()
        if (!discussionDoc.exists) {
            throw new functions.https.HttpsError('failed-precondition', 'discussion not found')
        }
        const discussion = { id: discussionDoc.id, ...discussionDoc.data() }

        // Fetch the membership document
        const membershipPath = `memberships/${uid}/junto-membership/${discussion.juntoId}`
        ...
        try {
            ...
            const [files] = await bucket.getFiles({ prefix: `${discussion.id}/` })
            files
